...
The distributions of daily rates rates and transactions transactions, billions of dollars, change dramatically under different FOMC policy regimes [figure times series of rates] (Figure \ref{fig:).  (Table \ref{table:FOMCratechanges} ). Data provided by the NY Federal Reserve download program summarize weighted average median rates and quantiles of these distributions. 
...
Changes in the Fed funds rate and the administered rates IOR and the reward rate ON RRP provides a summary of the data (\@ref(tab:FOMC rate changes 2016-2023) .
...
#C:/Users/Owner/Documents/Research/overnightRates/Final data files/Final data files/FOMCrates_sampleFinal.csv', header = TRUE, sep = ",", dec = ".", stringsAsFactors = FALSE)
# Set column names based on the first row
...
xtable(normstats, include.rownames = FALSE)
```
...
xtable(adjstats, include.rownames = FALSE)
```
...
xtable(charcovid)  #include.rownames = FALSE)
```
...
xtable(charzlb) #,include.rownames = FALSE)
```
...
xtable(zlbstats, include.rownames = FALSE)
```
...
xtable(charinflation) #include.rownames = FALSE)
```
