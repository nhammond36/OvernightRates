...
The improment provided by EGARCH models, volatility $\sigma^2_t$ responds asymmetrically to positive and negative residuals to accommodate returns volatility that rises in response to "bad news" (excess returns lower than expected) and falls in response to "good news" 
...
```{r,arima_bbp2,echo=FALSE, results='asis',warning=FALSE, message=FALSE}
length(non_trading_counts)
...
length(non_trading_counts)
length(log_sd_effr_squared)
# Use the optim function for optimization with a different method
result <- optim(par = gamma_start, fn = neg_log_likelihood, method = "BFGS")
# 
# Print the result
print(result)
...
#   nt <- log(1 - gamma_estimated * non_trading_counts)
#   length(non_trading_counts) #[1] 1957
#   print(nt)
#   length(nt)
#   
#   external_regressors <- cbind(h[,2:ncol(h)], z,nt,absnu, nu)
...
#   results <- data.frame(Coefficients = arima_params, StdErrors = std_errors)
  # print(results)    
#                     Replication 2016-2023             bbp_params             bbp1994_params
#                         Coefficients   StdErrors  Coefficients   StdErrors   Coefficients   StdErrors   # ar1                     0.844725586 0.013027682                               0.600 (0.038)
# intercept               1.995289332 0.107038918
# oneday_beforeholiday    0.090674060 0.132469988   -0.026 (0.017)
# threeday_beforeholiday -0.100208356 0.045532230   -0.018 (0.008)
# oneday_afterholiday     0.056307993 0.130075255    0.062 (0.018)
# endquarter              0.143136334 0.089780256    0.206 (0.034)              2.081 (0.181)
# endyear                 0.108245069 0.173393200   -0.183 (0.078)              2.913 (0.331)
# Monday                 -0.002832700 0.061330489
# Friday                 -0.013389683 0.023629359
# fomc                    0.009347913 0.005977139
# fomcindex              -0.284634005 0.148983964                               0.713 (0.262)
# z                      -0.700228944 0.221628438
# nt                      0.049702753 0.091452890
# absnu                   0.016534819 0.010158805                               0.718 (0.069)
# nu                     -0.002099820 0.006963208                               0.276 (0.042)
```
  
  

(Table (\@ref(tab:garch_bbp))
```{r,garch_bbp, echo=FALSE, results='asis',warning=FALSE, message=FALSE}
#eval=FALSE}  
...
<!--
3709-3750 [egarch_bbp] (ONRates_simplev10_08042024.rmd)
Error:
! object 'arima_residuals' not found
-->

(Table (\@ref(tab:egarch_bbp))
```{r,egarch_bbp,  echo=FALSE, results='asis',warning=FALSE, message=FALSE}
# Specify the EGARCH model --------------------------------------
  spec <- ugarchspec(
    variance.model = list(model = "eGARCH", garchOrder = c(1, 1)),
    mean.model = list(armaOrder = c(0, 0), include.mean = FALSE),
...
Mean Model : ARIMA(1,0,1)
Distribution : student t
  
$\alpha$ captures the sign effect
$\beta$  captures persistence
$\gamma$ size
shape parameter typically refers to parameters that control the distributional characteristics of the error terms. Specifically, it often pertains to the skewness and kurtosis of the distribution used to model the error terms.

The GARCH order is defined by (q; p) (ARCH, GARCH), with possibly m external regressors $x_j$
$ z=\epsilon $ residuals from the mean filtration process discussed previously. the standardized residual.

Related to this measure is the 'half-life' (call it h2l) defined as the number of days it takes for
half of the expected reversion back towards $E(\sigma^2)$ to occur. $P=\sum_{k=1}^{p}\beta_j$
...
\begin{equation}
1.7 log(\sigma$2_t)= \omega* + \sum_{k=1}^{\infty}\fi_j z^2_{t-j} \sigma^2_{t-j}

residuals, GARCH models elegantly capture the volatility clustering in asset returns first noted by Mandelbrot (1963): " .. . large changes tend to be followed by large changes-of either sign-and small changes by small changes... " This feature of GARCH models accounts for both their theoretical appeal and their empirical success. On the other hand, the simple structure of (1.7) imposes important limitations

We adopt another natural device for ensuring that ot2 remains nonnegative, by making ln (-t2) linear in some function of time and lagged $z_t$'s. That is, for some suitable function g:
\begin{equation}
2.1 log(\sigma$2_t)=\alpha_t + \sum_{k=1}^{\infty}\beta_k g(z_{t-k})
\end{equation}
$\beta_1 \equiv 1

To accommodate the asymmetric relation between stock returns and volatility changes noted in Section 1, the value of g(z,) must be a function of both magnitude and the sign of $z_t$. One choice, that in certain important cases turns out to give o-7 well-behaved moments, is to make $g(z_t)$ a linear combination z_t$ and $abs(z_t)$: 

\begin{equation}
(2.2) g(z_t) \equiv   \theta z_t + \gamma[abs{z_t) -E(abs(z_t))]
\end{equation}

By construction, ${g(z_t)}_{t=-\infty, \infty}$= is a zero-mean, i.i.d. random sequence. The two components of $g(z_t)$ are $\theta z_t $ and $\gamma[abs{z_t) -E(abs(z_t))]$, each with mean zero. If the distribution of $z_t$ is symmetric, the two components are orthogonal, though of course they are not independent. Over the range $0 < z_t < \infty$, g(zt) is linear in $z_t$ with slope $\theta + \gamma$, and over the range $- \infty <z_t < 0$, $g(z_t)$ is linear with slope $\theta - \gamma$. Thus, $g(z_t)$ allows the conditional variance process ${\sigma^2_t}$ to respond asymmetrically to rises and falls in stock price.

of the GARCH models discussed in Section 1, assume for the moment that $\gamma > 0$ and $\theta = 0$. The innovation in $ln (\sigma^2_t)$ is then positive (negative) when magnitude of $z_t$ is larger (smaller) than its expected value. Suppose now y = 0 and 0 < 0. The innovation in conditional variance is now positive (negative) when returns innovations are negative (positive). Thus the exponential form of ARCH in (2.1)-(2.2) meets the first objection raised to the GARCH models in Section 1.
...
Fit:
The inforcriteria method on a fitted or filtered object returns the Akaike (AIC), Bayesian (BIC), Hannan-Quinn (HQIC) and Shibata (SIC) information criteria to enable model selection
