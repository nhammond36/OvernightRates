...
header-includes:
  ...
  - \usepackage{float}
echo: false  
# Make sure this line is present and set to false
---
...
Machine learning ini R
\url{https://www.r-bloggers.com/2022/02/beginners-guide-to-machine-learning-in-r-with-step-by-step-tutorial/}
...
#sdate[endn] 
#"2019-07-31" "2019-10-31" "2020-03-16" "2022-03-16" "2023-12-14" "2023-12-14"
#sdate[begn] 
#"2016-03-04" "2019-08-01" "2019-11-01" "2020-03-17" "2022-03-17" "2016-03-04"
...
# Introduction
Piazzesi:
1. Meeting days of the Federal Open Market Committee (FOMC) are
marked as special events on the calendars of many market participants.
FOMC announcements often cause strong reactions in bond and stock
markets. Indeed, a large literature on announcement effect

Overnight rates interesting features:  cluster around FFR, quickly respond to  changes 
in FFR, IOR, ONRRP.  Market microstructure
2.These observations suggest that models of the yield curve should take
into account monetary policy actions by the Federal Reserve. The extensive term structure 
literature in finance, however, builds models
around a few unobservable state variables, or latent factors, which are
backed out from yield data. 

That they differ strongly in central tendencies, clustering, outliers under different policy 
regimes beg we understand the role of monetary shocks.  A further question posed by Bertolini 
et al, what are the Fed preferences for volatility in the FFR as it manages the FFR within 
ranges set by the FOMC. Relation to macroeconomy

3. data.
Method: volatility model with jumps from MP and announcements
The above observations also suggest that vector autoregressions
(VARs) in macroeconomics that try to disentangle exogenous policy
shocks from systematic responses of the Federal Reserve to changes in
macroeconomic conditions should take into account yield data

4.Models
5.Recent literature
How the Federal Reserve Bank (Fed) adjustment of reserves affects volatility of the Federal 
Funds rate (FFR) and wholesale overnight money market rates. What determines the Fed preferences  
for offset volatility in these rates in managing the FFR within the FOMC target range. The FFR is 
interest rate at which depository institutions or ”banks” lend funds to each other on an overnight 
basis. 
...
n the 1950s, Wall Street dealers originated the non deposit deposit, a repurchase transaction 
or repo, secured borrowing backed by collateral.The borrower issues a cash equivalent liability 
to the lender. In a reverse repo an entity lends cash against secured collateral. Since the 1970s, 
private parties have created deposit like instruments,  repos and money market funds ,intermediating credit outside the traditional banking system. This allows them to evade regulatory hurdles. 
...
Repo markets trade $4 trillions of dollars daily. Participants in  this tri repo market include 
primary dealers, broker-dealers, [get figure, note from graph]. Primary dealers intermediate US 
Treasury issuance to their clients by borrowing cash overnight from the Federal reserve, secured 
by Treasuries as collateral, then repurchasing the next day. In the 1990s, Section 13 (3) of the 
Federal Reserve act was amended to backstop dealers. Lenders include asset managers, money market  
funds, hedge funds, and security lenders including banks
...
The SOFR,  a broad measure of the cost of borrowing cash overnight collateralized by Treasury 
securities. replaces LIBOR as a benchmark overnight rate. The SOFR includes all trades in the 
Broad General Collateral market plus bilateral Treasury repurchase agreement (repo) transactions 
cleared through the Delivery-versus-Payment (DVP) service offered by the Fixed Income Clearing 
Corporation (FICC). The FUUCA trades are which filtered to remove a portion of transactions considered “specials”. 
...
By trading securities with banks and other counterparites, ederal Reserve purchases or sales of 
...
The distributions of daily rates rates and transactions transactions, billions of dollars, change dramatically from year to year.[figure times series of rates] (Figure \ref{fig:volumesample1})Data provided by the NY Fed data download program summarize, weighted average median rates, summarize 
other quantiles of these  distributions (Figure \ref{fig:samplechar}).

There are also major difference within FOMC policy regimes (Table \ref{table:FOMCdailyratesstats} ). 
Here volatility  as measured by the annual standard deviation of the series.
-->

Reserves went through a full expansion-contraction cycle from 2010 to late 2019 and expanded 
again in early 2020, ranging from 8$\%$ (2010 and 2019) to 19$\%$ (2014 and 2021) of banks’ 
assets. These movements reflect the Federal Reserve balance-sheet expansions in response to 
...
reverse repo to offset high frequency liquidity shocks to keep federal funds rate close to target.?stabilize FF around target rate..
...
- maintenace periods, calendar days, end of quarter, end of year, before and after holidays, 
large rate changes
...
\url{https://www.statista.com/statistics/187616/effective-rate-of-us-federal-funds-monthly/}
<!--
...
fomc_data <- data.frame(
  Period = c("2015 to 2018: Returning to Normalcy", "", "", "", "", "", "", "", "", "", "2019 Mid-Cycle Adjustment", "", "", "", "", "", "", "", "2020 Coping with Covid", "", "", "", "", "", "2022 Taming Inflation", "", "", "", "", "", "", "", "", ""),
  Date = c("20-Dec-18", "Sept. 27, 2018", "Jun. 14, 2018", "22-Mar-18", "Dec. 14, 2017", "15-Jun-17", "16-Mar-17", "Dec. 15, 2016", "Dec. 17, 2015", "", "31-Oct-19", "Sept. 19, 2019", "Aug. 1, 2019", "", "", "", "", "", "16-Mar-20", "3-Mar-20", "", "", "", "", "", "15-Dec-22", "02-Nov-22", "21-Sep-22", "27-Jul-22", "16-Jun-22", "05-May-22", "17-Mar-22", "27-Jul-23", "04-May-23", "23-Mar-23", "02-Feb-23"),
  Change = c(25, 25, 25, 25, 2, 25, 25, 25, 25, "", -25, -25, -25, "", "", "", "", "", -25, -50, "", "", "", "", "", 50, 75, 75, 75, 75, 50, 25, 25, 25, 25)
)

# Create the table using xtable
...
February 2, 2023: It adds another 0.25% increase to 4.50–4.75%
March 23, 2023: The Fed increases interest rates by an additional 0.25% to 4.75–5.0% 
and launches the Bank Term Funding Program, which will aid distressed banks suffering 
from interest-rate risk
May 4, 2023: The Fed hikes another 0.25% to 5.00–5.25%
July 27, 2023: The Fed delivers its final 0.25% increase of 2023, bringing rates to 
5.25–5.50%
...
-122 March 15, 2020 On March 15, 2020, the Fed shifted the objective of QE to supporting 
...
$\$$200 billion in government-guaranteed mortgage-backed securities over “the coming months.” 
 - 123 3/22/2020  March 23, 2020, it made the purchases open-ended, saying it would buy 
...
Time series propertie of overnight policy, interbank rates, transactions, and reserves 
held at the Federal Reserve
We examine the time series properties of overnight reference rates and their correspondence 
with the Federal Funds rate, the FOMC policy rate from 2016 through 2023.Transactions, reserves.
...
The rate data from the Federal Reserve Data Download Program are average volume weighted medians and select percentiles. The median, a measure of central tendency, and quantiles summarize the distribution of rates characterized be extreme value and outliers (\@ref(tab:sampletable_characteristics).
...
```
```
Combine the matrices into one matrix [DISASTER]
(see Figure @ref(fig:Overnight rates quantiles 2016-2023)
```{r Overnight rates quantiles, daily sample
quantile dataframes, echo=FALSE, cache=FALSE,results=‘hide’} 
boxplot_sample <- data.frame(
Group = structure(1:4, .Label = c(“EFFR”, “TGCR”, “BGCR”, “SOFR”), class = “factor”), 
y0 =
c(min(quantilesE𝐸𝐹𝐹𝑅), 𝑚𝑖𝑛(𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒𝑠𝑇 TGCR), min(quantilesB𝐵𝐺𝐶𝑅), 𝑚𝑖𝑛(𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒𝑠𝑆SOFR)),
y25 = c(quantile(quantilesE𝑃 𝑒𝑟𝑐𝑒𝑛𝑡𝑖𝑙𝑒25𝐸𝐹𝐹𝑅, 0.25), 𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒(𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒𝑠𝑇Percentile25_TGCR,
0.25), quantile(quantilesB𝑃 𝑒𝑟𝑐𝑒𝑛𝑡𝑖𝑙𝑒25𝐵𝐺𝐶𝑅, 0.25), 𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒(𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒𝑠𝑆Percentile25_SOFR, 0.25)),
y50 = c(median(quantilesE𝐸𝐹𝐹𝑅), 𝑚𝑒𝑑𝑖𝑎𝑛(𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒𝑠𝑇 TGCR), median(quantilesB𝐵𝐺𝐶𝑅), 𝑚𝑒𝑑𝑖𝑎𝑛(𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒𝑠𝑆SOFR)),
y75 = c(quantile(quantilesE𝑃 𝑒𝑟𝑐𝑒𝑛𝑡𝑖𝑙𝑒75𝐸𝐹𝐹𝑅, 0.75), 𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒(𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒𝑠𝑇Percentile75_TGCR,
0.75), quantile(quantilesB𝑃 𝑒𝑟𝑐𝑒𝑛𝑡𝑖𝑙𝑒75𝐵𝐺𝐶𝑅, 0.75), 𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒(𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒𝑠𝑆Percentile75_SOFR, 0.75)),
y100 = c(max(quantilesE𝐸𝐹𝐹𝑅), 𝑚𝑎𝑥(𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒𝑠𝑇 TGCR), max(quantilesB𝐵𝐺𝐶𝑅), 𝑚𝑎𝑥(𝑞𝑢𝑎𝑛𝑡𝑖𝑙𝑒𝑠𝑆SOFR)))
DELETE CHUNK
```
...
See (\@ref(tab:sampletable_metrics) provides a summary of the data.
...
```{r, full sample log percent change repeat, echo=FALSE}
#Full sample
...
\@ref(tab:normtable_characteristicsV2) provides a summary of the data.
\```{r normtable_characteristicsV2, tab.cap="Rate characteristics normalcy 3/4/2016-7/31/2019", echo=FALSE, results='asis'}
data <- data.frame(
...
tryCatch({
  ...
  # Print error message in a format that LaTeX can handle
  cat("Error creating normstats: \\textbackslash n", e$message, "\\textbackslash n")
})
```
...
```{r, EFFR during adjustment period, echo=FALSE}
# By ensuring that both the data chunk and the figure chunk use the same label (`EFFR during adjustment #period`), it will be easier to understand the relationship between the data preparation and the resulting #figure. This will also ensure that the LaTeX reference matches the correct figure caption.
...
rates_adjust <<- ggplot(meltrates_adjust, aes(x = sdate, y=value, colour=variable, group=variable)) +
  geom_line() +
  labs(x = "", y = "rates basis points (bp), volume $billion", color = "Rate", linetype = "Rate") +  
  scale_y_continuous(breaks = seq(minr, maxr, by = 25), limits = c(minr, maxr)) + 
...
ratests_covid <- ggplot(meltrates_covid,aes(x=sdate,y=value,colour=variable,group=variable)) + 
  ...
  scale_y_continuous(breaks = seq(20, maxr, by = 10), limits = c(20, maxr)) + 
 # scale_y_continuous(breaks = seq(0, mxq, by = .05), limits = c(0, mxq)) + 
  theme_minimal() + guides(shape = guide_legend(title = "Rate"))
print(ratests_covid)
```
...
maxr<-max(qcovidE[,rates]) #160
minr<-min(qcovidE[,rates])  #25
```
...
rates_zlbts <- ggplot(meltrates_zlb,aes(x=sdate,y=value,colour=variable,group=variable)) + 
  geom_point(shape=16,size=1) +
...
<!--
Chat It sounds like there might be some unexpected interaction or linking between the data chunks and the plot chunks in your R Markdown document. This could be causing issues with the rendering of the plots and the generation of LaTeX code.

Here are a few steps you can take to troubleshoot and address the issue:

Check Chunk Labels: Ensure that each data chunk and its corresponding plot chunk have unique and consistent labels. The labels used in the fig.cap option should match the labels used in the chunk options ({r}) for the plot chunks.

Chunk Dependencies: Review the dependencies between different chunks in your R Markdown document. Ensure that each plot chunk has the correct data chunk specified as its dependency using the knitr::opts_chunk$set function or the dependson chunk option.

Chunk Options: Check the options specified in each chunk to ensure they are correct and consistent. Pay attention to any custom options or settings that might affect the behavior of the chunks.

Chunk Separation: If the plots are being combined or linked unexpectedly in the LaTeX output, try adding more separation between the chunks by inserting additional text or markup between them. This can help ensure that each chunk is processed independently.

Data Chunk Content: Review the content and structure of the data chunks to ensure they are correctly formatted and do not contain any syntax errors or unexpected values. If necessary, consider simplifying the data or restructuring it to make it more compatible with the plot chunks.

Debugging: If you suspect problems with specific data chunks, try running each chunk individually to isolate any errors or issues. Use print statements or other debugging techniques to inspect the data and identify any potential problems.

By carefully reviewing and addressing these factors, you should be able to identify and resolve any issues causing unexpected linking or errors in your R Markdown document. If you continue to encounter problems, feel free to provide more details or specific sections of your document for further assistance.
-->
...
[WHAT ARE THESE, QUANTILES ACCOMPANY BOX PLOTS]
...
maxr<-max(inflation[,2:ncol(inflation)]) #224
minr<-0
#Create a data frame including 'sdate' and melt it
...
CHECK duplicates at end, inflation plot better

<!--
sdate \(\leftarrow\) as.Date(qadjE[, 1])
Similarly, replace all other instances of \textless- with \(\leftarrow\) or \leftarrow in your LaTeX code.
-->
...
(see Figure \@ref(fig:EFFR Key percentiles during normalcy period 3/4/2016-7/31/2019 -plot, echo=FALSE)
```{r, data EFFR Key percentiles during normalcy period 3/4/2016-7/31/2019}
qnormE=quantilesE[bgn:edn,] # rate specific metrics  -->
qnormT=quantilesT[bgn:edn,] 
qnormB=quantilesB[bgn:edn,] 
qnormS=quantilesS[bgn:edn,] 
...
```
(see Figure \@ref(fig:EFFR Key percentiles during normalcy period 3/4/2016-7/31/2019 -plot)
...
sdate position
<!--
Session Environment: Check if there are any differences in the session environment between your console and the Rmarkdown document. Sometimes, certain packages or variables may not be loaded or available in the Rmarkdown session. You can use sessionInfo() in both environments to compare the session details.

Package Loading: Ensure that all necessary packages are loaded at the beginning of your Rmarkdown document. If you're using any custom functions or packages, make sure they are loaded or sourced properly.

File Paths: Double-check the file paths used in your Rmarkdown document. Relative paths may behave differently depending on the working directory when knitting the document. Using absolute paths can help ensure consistency.

Debugging: Try adding print statements or intermediate checks in your Rmarkdown chunk to see the values of variables at different stages. This can help identify where the issue is occurring.

R Markdown Output: If possible, try generating a minimal reproducible example of the issue and share it here. It might help in diagnosing the problem more accurately.

By carefully examining these aspects and comparing the behavior between the console and the Rmarkdown document, you should be able to pinpoint the source of the error and resolve it.

some issue with the data type conversion.

Here are a few steps to troubleshoot and resolve this issue:

Check Data Types: Ensure that the columns of your qadjE dataframe are indeed numeric. You can use the str() function in R to inspect the structure of the dataframe and verify the data types.

Data Import: If the data is not being read correctly, double-check the data import process. Make sure that the date column is being parsed as dates and numeric columns are being correctly identified as numeric.

Data Cleaning: If there are any non-numeric values in numeric columns, they could prevent the conversion to numeric. You may need to clean your data to ensure all values in numeric columns are indeed numeric.

Debugging in Rmarkdown: Try running the code within your Rmarkdown document line by line or in smaller chunks to identify where the issue is occurring. This can help pinpoint the specific line of code causing the error.

Error Messages: Pay close attention to any error messages or warnings that Rmarkdown provides. They often give clues about what might be going wrong.

Once you've identified and addressed the issue causing the data type discrepancy, your Rmarkdown document should be able to execute the code without errors.
...
```{r , data EFFR Key percentiles during mid cycle adustment period 8/1/2019-10/31/2019, echo=FALSE}
bgn=859
edn=922
...
(see Figure \@ref(fig:EFFR Key percentiles during mid cycle adustment period 8/1/2019-10/31/2019-plot)
...
```{r , data EFFR Key percentiles during covid period 8/1/2019-10/31/2019, echo=FALSE}
...
Sstatscovid2<-c(Sstatscovid[1],Sstatscovid[2], TargetUe=NA, TargetDe=NA,Sstatscovid[3],Sstatscovid[4],Sstatscovid[5],Sstatscovid[6])
```
(see Figure \@ref(fig:EFFR Key percentiles during covid period 11/1/2019-3/16/2020  -plot)
...
(see Figure \@ref(fig:EFFR Key percentiles during zero lower bound  period 3/4/2016-7/31/2019 -plot)
```{r, data EFFR Key percentiles during zero lower bound period 3/16/2022,echo=FALSE}
k=4
begn<-c(1,859,923,1014,1519,1)
endn<-c(858,922,1013,1518,1957,1957)
bgn<-begn[k]
...
(see Figure \@ref(fig:EFFR Key percentiles during zero lower bond period  3/17/2020-3/16/2022-plot)
...
```
```{r , data EFFR Key percentiles during inflation period 03/17/2022-12/14/2023, echo=FALSE}
k=5 
begn<-c(1,859,923,1014,1519,1)
endn<-c(858,922,1013,1518,1957,1957)
bgn<-begn[k]
...
Estatsinflation <<- colMeans(qinflationE[,1:ncol(qinflationE)-1], na.rm = TRUE)
...
(see Figure \@ref(fig:EFFR Key percentiles during inflation period 3/17/2022-12/14/2023-plot)
...
The Duffie-Krishnamurthy seems to track the clustering over overnight rates (see Figure \@ref{fig:Duffie Krishnamurthy index}). The index reaches a maximum of 401.12 on 2023-12-01, a low of 10.03 2018-03-30. 

<!--
max(dkindex)
[1] 401.1328  "2023-12-01"  row 1948
> min(dkindex)
[1] 10.03117 sdate[522]
[1] "2018-03-30"
-->

REVIEW row <- which(dkindex==401.1328) 
In our sample from 2016 through 2023, the Duffie Krishnamurthy diversity index registers low dispersion, under 4 basis points, among the five overnight rates  EFFR, OBFR, TGCR, BGCR, and SOHR during 2016 and the first quarter of 2017 before jumping to around 5 during the last 2 weeks of March 2017 
...
garaindexdf$group <- rep(c("Alonso index", "Lower target FFR", "Upper target FFR", "EFFR", "TGCR", "BGCR", "SOFR"), length.out = nrow(garaindexdf))
...
ggara <- ggplot(garaindexdf, aes(x = sdate, y = g, color = group)) +
  geom_point(shape = 16, size = 1) +
...
table:weeklyreserves' on page 66 undefined on input line 3289.
Warning: LaTeX Warning: Reference `fig:weeklyreserves' on page 66 undefined on input line 3289.
Warning: LaTeX Warning: Reference `table:statsweeklyratesannual' on page 66 undefined on input line 3297.
Warning: LaTeX Warning: Reference `figure' on page 66 undefined on input line 331
