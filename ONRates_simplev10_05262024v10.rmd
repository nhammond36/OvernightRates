...
Making $ln(\sigma^2_t)$ a linear function of time and lagged z's, ensures that $\sigma^2_t$ remains
nonnegative,
...
\begin{align*}
 log(\sigma^2_t)-\xi_t - \kappa h_t& =\sum_{j=1}^{T} \delta_j log(\sigma^2_{t-j})- \xi_{t-j} - \kappa h_{t-j} +  \alpha(abs(v_{t-1} -E[abs(v_{t-1})]+ \chi v_{t-1}))\
\end{align*} (6)
...
$z_t$ i.i.d. with $E(z_t) =0, Var(z_t) = 1$
\begin{align*}
...
I will replicate Bertola et al EGARCH for comparison with other methods. 
Bertolini, Bertola, Prati () Time series methodology: 
...
calendar effect $h_t$ and $z_t$ target rate as proportion of penalty rate (discount rate?)
