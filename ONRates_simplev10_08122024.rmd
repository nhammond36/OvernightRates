...
This paper describes the dynamics of these rates and proposes a model of volatility joint with monetary policy. How that volatility may vary under different monetary policy regimes may reveal the Fed's preferences for offsetting volatility in managing the FFR with the FOMC targets.


# Introduction
  W examine how monetary policy impacts volatility of rates in the market for repurchase agreements may reveal the Federal Reserve Bank (Fed) preference for volatility as it manages the policy rate, the Federal Funds rate (FFR) with the target rates specified by the FOMC. The Fed's  open market operations adjust reserves through repurchase transactions to offset shocks the Federal Funds rate. How does the Fed manage the FFR to survive the volatility of shocks? 
...
the excess liquidity created by quantitative easing after the Great Financial Crisis (GFC) made the Fed's job of manage its policy rate difficult.  Events like the spikes in repo and the FFR in 2019, the March 2020 dash for cash show how the adjustment of reserves through repurchase markets and the arbitrage opportunities between repo and the price of reserves affect credit and the economy. By increasing reserves through repurchases, the Fed can lower the FFR  or raise the FFR by lending reserves in reverse repurchase transactions.
...
The tri-party general collaterial market is repo’s primary funding market for cash lenders or borrowers. For example, primary dealers are required to intermediate US Treasury issuance to their customers. They finance these purchases from their inventory or by borrowing from the Fed in the tri-party market. Participants obtain cash at the cheapest rates primarily from MMFs (money market funds) in tri-party repo, then lend at a spread in other repo markets. Primary dealers sell securities to the customers in the more lucrative OTC broad general collateral market.
...
However, there are many compendia of shocks to draw on.  I will evaluate each one when choosing methodality to model the volatility of the FFR and repo market rates.
...
(Kerssenfischer, Schmeling (2023) compile a vast database from to 2020 of news that impact asset prices: macroeconomic data releases, monetary policy announcements, and ad  hoc news.

The find the they regress the second moment of asset returns y, bonds and stocks, on dummy variables representing news sources, time fixed effects such as days, and lagged volatility measures..
...
The fraction of total market movements they attribute to news:
\begin{align*}
\Omega(k) = \sum_{i=1}^{k}\frac{E[y^2_t|D_{i,t}=1] P(D_{i,t=1}}{E(y^2)}
\Omega(k) = \sum_{i=1}^{k}\frac{\beta_i P(D_{i,t=1}}{E(y^2)}
\end{align*}

where k is the rank of each news release for a given asset (for most assets, k = 1 is the US employment report. The $betas$ in Eq. (2) correspond to estimates of the effect of news i on the conditional second moment. Weighing each coefficient by the frequency of releases and summing over all news gives us the second-moment share explained by all news. Since E[y] $\approx 0$ in tight event windows as discussed above, this measure is very close to a variance share and we thus label it $R^2$.
<!--
...
# Methodology
[Should I put percent change sigma here and comment on results?]
