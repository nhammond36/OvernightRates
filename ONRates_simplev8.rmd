...
# Introduction
Daily trillions of dollars are transacted in :the market for repurchase agreements.The repo market has grown into a $4 trillion-a-day funding market to provide liquidity to he Fed’s primary dealers, large broker-dealers, and other financial participants GSEs, MMF, Primary dealers play an important function intermediating US Treasury debt through these markets.
...
Bank reserves and deposits swapped for Treasuries and other securities are then swapped for repos used to fund loans, obtain leverage, and many other functions. today’s repo markets make Treasuries and other government securities virtually equivalent to cash.
...
It is no surprsie that that he whole money market rates to gethern with vernight reference rates, the Federal Reserve's policy and administrative rate share many features: clustering around the Federal Funds rate (FFR), quick response to changes in FFR and administrative rates, interest on reserves (IOR), the overnight reverse repo rate (ONRRP), large outliers, and heteroskedastcity, volatility that changes over time.
...
•	Fed opens its RRP (reverse repo) facility and accepts offers to lend cash against securities from its mammoth portfolio.SOMA
...
The Fed opened its RRP (reverse repo) facility and accepts offers to lend cash against securities from its SOMA portfolio.
...
•	In 2019, repo, Fed Funds, and FX swap rates spiked during a cash shortage, prompted by scarce bank reserves, constrained dealer balance sheets, and a segmented repo market, surpassing the Fed’s upper rate boundaries. 
...
dates <- c("3-Mar-20", "11-Mar-20", "15-Mar-20", "16-Mar-20", "Apr-20", "Jun-20", "28-Aug-20", 
           "Dec-20", "28-Jul-21", "Nov-21", "December 2021", "26-Jan-22", "24-Feb-22", "16-Mar-22", 
           "5-May-22", "Jun-22", "16-Jun-22", "28-Jul-22", "22-Sep-22", "3-Nov-22", "15-Dec-22", 
           "2-Feb-23", "23-Mar-23", "4-May-23", "27-Jul-23")
...
notes<- data.frame(Date = as.Date(notes_with_valid_dates$Date, origin = "1970-01-01"), 
                       Event = c("Fed emergency rate cut",
...
# Start here, delete above when sure -------------------------------------
# Load the lubridate package
library(lubridate)
# Create a dataframe with 'Date' and 'Event' columns

# Start here May 11, add lines for notes df aboce
# Create a function to parse dates flexibly
# Load the lubridate package

# if we can use formats June 2020
dates <- c("June 2020", "December 2020", 
           "March 3, 2020", "March 11, 2020", "March 15, 2020", "March 16, 2020", 
           "April 2020", "August 28, 2020", "July 28, 2021", "November 2021", "December 2021", 
           "January 26, 2022", "February 24, 2022", "March 16, 2022", "May 5, 2022", "June 2022", 
...
dates <- c("June 1, 2020", "December11, 2020", 
           "March 3, 2020", "March 11, 2020", "March 15, 2020", "March 16, 2020", 
           "April 2020", "August 28, 2020", "July 28, 2021", "November 1, 2021", "December 1, 2021", 
...
notes_with_valid_dates <- notes[valid_indices & !is.na(parsed_dates), ]
# why does this have 24 obs, 2 vairables Date  Event
...
#____________________ so I dont need this? ------------------------
notes <- data.frame(
...
# Filter notes dataframe to include only rows with valid parsed dates
notes_with_valid_dates <- notes[which(valid_indices), ]

# Parse dates directly and assign to Date column in filtered notes dataframe
notes_with_valid_dates$Date <- as.Date(notes_with_valid_dates$Date, origin = "1970-01-01")


# Create a dataframe with 'Date' and 'Event' columns
notes_df <- data.frame(Date = notes_with_valid_dates$Date, Event = notes_with_valid_dates$Event)
...
#
# Create a dataframe with 'Date' and 'Event' columns
notes_df <- data.frame(Date = as.Date(notes_with_valid_dates, format = "%B %d, %Y"), Event = notes_with_valid_dates)

# Filter valid indices from parsed dates
valid_indices <- !is.na(parsed_dates)

# Filter notes dataframe to include only rows with valid parsed dates
notes_with_valid_dates <- notes_df[valid_indices, ]
