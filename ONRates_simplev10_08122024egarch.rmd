...
This paper describes the dynamics of these rates and proposes a model of volatility joint with monetary policy. How that volatility may vary under different monetary policy regimes may reveal the Fed's preferences for offsetting volatility in managing the FFR within the FOMC targets.
...
Under the corridor system that implements the  ample reserves, the Fed's setting of the IORB and O/N RRP establish  the ceiling and floor within which the Fed manages the range of the FFR. 
...
non_trading_counts <- non_trading_days_pairs(dates, holidays)
  ...
  ## AONLY DO ONCE IF YOUR START OVER!
  #djusting X to match log_sd_effr_squared
...
# delete this part
# Chat's refined approach
# neg_log_likelihood <- function(gamma) {
#     # Ensure gamma is numeric
#     gamma <- as.numeric(gamma)
#     if (is.na(gamma)) stop("gamma must be numeric")
#     
#     # Debug information
#     print(paste("gamma:", gamma))
#     print(paste("Length of non_trading_counts:", length(non_trading_counts)))
#     print(paste("Length of log_sd_effr_squared:", length(log_sd_effr_squared)))
#     
#     if (length(non_trading_counts) != length(log_sd_effr_squared)) {
#         stop("non_trading_counts and log_sd_effr_squared must be of the same length")
#     }
#     
#     if (any(1 - gamma * non_trading_counts <= 0)) {
#         return(Inf)
...
#     log_likelihood <- -sum(dnorm(residuals, mean = 0, sd = sd(residuals), log = TRUE))
#     
#     print(paste("gamma:", gamma, "log_likelihood:", log_likelihood))
#     
#     return(log_likelihood)
# }
...
str(non_trading_counts) #num [1:1957] 0 2 0 0 0 0 2 0 0 0 ...
summary(non_trading_counts)
 #Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #0.0000  0.0000  0.0000  0.4149  0.0000  2.0000 
# 
 str(log_sd_effr_squared) # num [1:1957] 2.2 1.39 2.2 1.39 1.39 ...
 summary(log_sd_effr_squared)
 # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 # 0.000   1.386   2.197   1.757   2.197   7.222 
 
# 
#   gamma_estimated <- result$par
# #   Here's a breakdown of the components in the result list:
# # par: The optimal parameter found (gamma_value = 0.01).
# # value: The value of the negative log likelihood at the optimal parameter (2873).
# # counts: The number of function and gradient evaluations used in the optimization.
# # convergence: An integer code indicating the convergence status (0 means successful convergence).
# # message: Any message from the optimization process (NULL means no message, which is typically a good sign).
...
```{r,garch_bbp, eval=TRUE,  echo=FALSE, results='hide', message=FALSE, warning=FALSE}
...
# Specify the EGARCH model --------------------------------------
  ...
  std_errors <- sqrt(diag(vcov_matrix))
  results_Egarch <- data.frame(Coefficients = coefficients, StdErrors = std_errors)
  print(results_Egarch)
#          Coefficients  StdErrors
# omega    0.01341569 0.24756022
# alpha1   0.04848064 0.08876039
# beta1    0.64179830 0.04568855
# gamma1   2.13728765 0.74535917
# shape    2.10000000 0.07590593
  
  # Convert the parameter_estimates to a data frame
  #coefgarch_df <- as.data.frame(results_Egarch)
  garch_bbp <- as.data.frame(results_Egarch)
...
#   Replication_Coefficients = c(0.844725586, 1.995289332, 0.090674060, -0.100208356, 0.056307993,
#                                0.143136334, 0.108245069, -0.002832700, -0.013389683, 0.009347913,
#                                -0.284634005, -0.700228944, 0.049702753, 0.016534819, -0.002099820),
#   Replication_StdErrors = c(0.013027682, 0.107038918, 0.132469988, 0.045532230, 0.130075255,
#                             0.089780256, 0.173393200, 0.061330489, 0.023629359, 0.005977139,
#                             0.148983964, 0.221628438, 0.091452890, 0.010158805, 0.006963208),
#   bbp_full_Coefficients = c(NA, NA, -0.026, -0.018, 0.062, 0.206, -0.185, NA, NA, NA, 0.431, NA, NA, NA, NA),
#   bbp_full_StdErrors = c(NA, NA, 0.017, 0.008, 0.018, 0.034, 0.078, NA, NA, NA, NA, NA, NA, 0.069, 0.042),
#   bbp1994_params_Coefficients = c(0.600, NA, NA, NA, NA, 2.081, 2.913, NA, NA, NA, 0.713, 1.24, 0.668, 0.718, 0.276),
#   bbp1994_params_StdErrors = c(0.038, NA, NA, NA, NA, 0.181, 0.331, NA, NA, NA, 0.262, 0.465, 0.195, 0.069, 0.042)
# )
# 
...
egarch_bbp
 ...
 # Create a table using xtable-------------------- for overnight EFFR
egarch_bbp_table <- xtable(egarch_bbp, caption = "EGARCH (1,1) 3/4/2016 to 12/14/2023 vs Bartolini, Bertoal, Prati", label="tab:egarch_bbp", caption.placement="bottom", comment = FALSE)
egarch_bbp_table
```     
...
<!-- But above doesnt have BBP's GARCH effects, shit!-->
<!--
results OLD?
                       Coefficients  StdErrors
ar1                     0.844725586 0.24756022
intercept               1.995289332 0.08876039
oneday_beforeholiday    0.090674060 0.04568855
threeday_beforeholiday -0.100208356 0.74535917
oneday_afterholiday     0.056307993 0.07590593
endquarter              0.143136334 0.24756022
endyear                 0.108245069 0.08876039
Monday                 -0.002832700 0.04568855
Friday                 -0.013389683 0.74535917
fomc                    0.009347913 0.07590593   .431 (0.050)
fomcindex              -0.284634005 0.24756022
z                      -0.700228944 0.08876039
nt                      0.049702753 0.04568855
absnu                   0.016534819 0.74535917
nu                     -0.002099820 0.07590593
-->

<!--
\emph{Comparison of findings} A summary of their findings:
The variance of the FFR rate in Bertolini et al full sample:
