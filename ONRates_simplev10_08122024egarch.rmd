...
<!-- 
Delete all these chunks if the new arima_bbp2 works!!
```{r, define_neg_log_likelihood, eval=FALSE, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
neg_log_likelihood <- function(params) {
  # Your function code here
}
```
 
```{r, define_gamma_start, eval=FALSE, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
# Define gamma_start as an initial guess
gamma_start <- 0.01
```

```{r,arima_bbp2,eval=FALSE, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
#echo=FALSE, results='asis',warning=FALSE, message=FALSE}
length(non_trading_counts)
length(log_sd_effr_squared)

non_trading_counts <- na.omit(non_trading_counts)
 log_sd_effr_squared <- na.omit(log_sd_effr_squared)
 #print(length(non_trading_counts)) #[1] 1957
 #print(length(log_sd_effr_squared)) #[1] 1957
 #print(head(non_trading_counts))
#[1] 0 2 0 0 0 0
print(head(log_sd_effr_squared))
#[1] 2.197225 1.386294 2.197225 1.386294 1.386294 1.386294

#NEW
neg_log_likelihood <- function(gamma_value) {
    # Ensure gamma_value is numeric
    if (!is.numeric(gamma_value)) {
        stop("gamma_value must be numeric")
    }
    
    # Debug information
    print(paste("gamma_value:", gamma_value))
    ...
    # Check for invalid values in the gamma_value calculation
    if (any(1 - gamma_value * non_trading_counts <= 0)) {
        return(Inf)
    }
    
    # Proceed with the calculations if no errors
    X <- log(1 - gamma_value * non_trading_counts)
    model <- lm(log_sd_effr_squared[2:length(log_sd_effr_squared)] ~ X[-1])
    residuals <- residuals(model)
    log_likelihood <- -sum(dnorm(residuals, mean = 0, sd = sd(residuals), log = TRUE))
    
    print(paste("gamma_value:", gamma_value, "log_likelihood:", log_likelihood))
    
    return(log_likelihood)
}

# Initial guess for gamma_value
gamma_start <- 0.01
...
# Augst 9
log_likelihood <- 0  # Replace with your actual log-likelihood calculation
...
#1 
test_gamma <- 0.01
result <- neg_log_likelihood(test_gamma)

#2
result <- optim(par = gamma_start, fn = function(g) {
    print(paste("Optim gamma:", g))
    neg_log_likelihood(g)
}, method = "BFGS")


# OH NO lost the end But I think its ok
...
```{r,garch_bbp, eval=FALSE,  echo=FALSE, results='hide', message=FALSE, warning=FALSE}
#echo=FALSE, results='asis',warning=FALSE, message=FALSE}
#eval=FALSE}  
...
# shape    2.10000000 0.07590593
  ...
  garch_bbp <- as.data.frame(results)
...
\emph{Findings Bertolini, Bertola, Prati (2000) } 
...
```{r,egarch_bbp, eval=FALSE, echo=FALSE, results='hide', message=FALSE, warning=FALSE}
#echo=FALSE, results='asis',warning=FALSE, message=FALSE}
# Specify the EGARCH model --------------------------------------
  spec <- ugarchspec(
    variance.model = list(model = "eGARCH", garchOrder = c(1, 1)),
