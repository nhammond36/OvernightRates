...
output: 
  bookdown::pdf_document2:
    toc: false # Specify whether a table of contents should be included
    keep_tex: true
    fig_caption: yes
    includes:  
...
Estimating the Fed's unconventional policy shocks Javorscinski (2022) estimates structural shocks that underlie the reactions of financial market to FOMC announcements. While the nature of the shocks is not specified ex ante, ex post the estimated shocks can be naturally labeled as the current fed funds rate policy, an “Odyssean” forward guidance (a commitment to a future course of policy rates), a large scale asset purchase and a “Delphic” forward guidance (a statement about the future course of policy rates understood as a forecast of the appropriate stance of the policy rather than a commitment (Campbell et al., 2012).

Alvarez metrics for kurtosis


# Time series properties of overnight reference rates and transactions
...
IORR-EFFR
Set by the Fed, the IORB rate is the return rate depository institutions can earn for holding reserves at the Federal Reserve. Meanwhile, the EFFR represents the cost of interbank borrowing and is determined by the market. The difference between these two rates present arbitrage opportunities for banks. Banks can borrow funds at a lower EFFR and deposit the funds for a higher IORB rate, making a profit on the spread.
...
SOFR
The SOFR is an influential interest rate banks use to price U.S. dollar-denominated derivatives and loans. The daily SOFR is based on transactions in the Treasury repurchase market, where investors offer banks overnight loans backed by their bond assets. SOFR replaced the London Interbank Offered Rate (LIBOR).
...
OBRRP and TBills
T-bills Outstanding Decreased from January 2021 to July 2022
...
% latex table generated in R 4.3.2 by xtable 1.8-4 package % Mon Jun 17 16:58:28 2024
You don't. The purpose of xtable is to output the raw LaTeX, which you then put into a .tex document and compile separately (or in a .rnw file and use knitr/sweave). – 
joran
 CommentedJul 15, 2014 at 17:49g
...
<!--
Chunk label: Use a simple and unique label like fomc-table.
Caption in xtable: Set the caption directly within the xtable function.
Reference the table: Use the same label you used in the chunk options with \@ref(tab:chunk-label).
...
fdate <- as.Date(fomc$Date, "%d/%m/%Y")
# Select the first 9 rows for the table
normtab <- fomc[1:9,]
...
print(xtable_norm, caption.placement = "top")


See the changes in interest rates in Table \@ref(tab:fomc-table).

-->


```{r ratestsnorm, fig.cap="Rates during normalcy period 3/4/2016-7/31/2019", echo=FALSE, warning=FALSE, message=FALSE,fig.pos='H', fig.label="ratestsnorm"}
library(ggplot2)
library(reshape2)
...
The EFFR and the money market rates during the covid period 
...
ratests_covid <- ggplot(meltrates_covid,aes(x=sdate,y=value,colour=variable,group=variable)) + 
  ...
  theme_minimal() + guides(shape = guide_legend(title = "Rate"))
#ratests_covid
ggsave(filename = "C:/Users/Owner/Documents/Research/OvernightRates/ONRates_simplev10_05262024v7_files/figure-latex/ratestscovid-1.pdf", plot = ratests_covid, device = "pdf")
```


```{r include_figure, results='asis', echo=FALSE}
cat('
\\begin{figure}[H]
  \\centering
  \\includegraphics[width=0.8\\textwidth]{C:/Users/Owner/Documents/Research/OvernightRates/ONRates_simplev10_05262024v7_files/figure-latex/ratestscovid-1.pdf}
  \\caption{Rates during covid period 11/1/2019-3/16/2020}
  \\label{fig:ratestscovid}
\\end{figure}
')
```
...
The EFFR and the money market rates during the zero lower bound 
...
```{r include_figurezlb, results='asis', echo=FALSE}
cat('
\\begin{figure}[H]
  \\centering
  \\includegraphics[width=0.8\\textwidth]{C:/Users/Owner/Documents/Research/OvernightRates/ONRates_simplev10_05262024v7_files/figure-latex/ratestszlb-1.pdf}
  \\caption{Rates during Zero lower bound 3/17/2020-3/16/2022}
  \\label{fig:ratestszlb}
\\end{figure}
')
```
...
The EFFR and the money market rates during the taming of inflation 
...
The difference in the central tendencies as measured by the median rate, changes dramatically over different policy episodes. 
...
(Table (\@ref(tab:samplese))
```{r,samplese,  echo=FALSE, results='asis',warning=FALSE, message=FALSE}
se_data <- matrix(c(
  0.00230737, 0.07063578, 0.0706312, 0.07104168,
  1.68997540, 2.07038809, 2.0636295, 2.08888530,
  1.62121041, 1.99282741, 1.9865812, 2.01114320,
  0.12690438, 0.65372007, 0.6540618, 0.65414530,
  0.19958811, 2.48072718, 2.4806234, 2.49107194
), nrow = 5, byrow = TRUE)

# Assigning row and column names
rownames(se_data) <- c("EFFR", "TGCR", "BGCR", "SOFR", "ones_v")
colnames(se_data) <- c("EFFR", "TGCR", "BGCR", "SOFR")
se <- as.data.frame(se_data)
#print(se)

sesample<-xtable(se,caption = "Std errors sample rates 3/4/2016 to 12/14/2023", label="tab:samplese", caption.placement="bottom", comment = FALSE)
...
(Table (\@ref(tab:samplef))
```{r,samplef,  echo=FALSE, results='asis',warning=FALSE, message=FALSE}
ft_data <- matrix(c(
805672.2,  5,  0,
186463.2,  5,  0,
186503.8,  5,  0,
...
rownames(ft_data) <- c("EFFR", "TGCR", "BGCR", "SOFR")
colnames(ft_data) <- c("F", "df", "P value")
ft <- as.data.frame(ft_data)
#print(ft)

fsample<-xtable(ft,caption = "F test sample rates 3/4/2016 to 12/14/2023", label="tab:samplef", caption.placement="bottom", comment = FALSE)
rownames(ft_data) <- c("EFFR", "TGCR", "BGCR", "SOFR")
colnames(ft_data) <- c("F", "df", "P value")
#ft <- as.data.frame(ft_data)
fsample
...
## Replicate of Bertola et al empirical model of the Fed Funds rate for comparison with other methods. 
Bertolini, Bertola, Prati () time series methodology for the Fed Funds rates aimed to: 
- capture mean reversion behavior
- establish main patterns of volatility in the data

Bertolini et al results of the volatility of the FFR
...
```{r,arima_bbp,  echo=FALSE, results='asis',warning=FALSE, message=FALSE}
...
```{r,arima_both,  echo=FALSE, results='asis',warning=FALSE, message=FALSE}
  # try 6 --------------------------------------------------
...
- rise at end of quarters 1,2, and 3 with variance 8 times larger that that of a typical day and falls after
...
- the distribution of errors df of student very fat tailed with t 2.94 significantly different than 3 $\rho=0.747$

This paper covers the years 2016 through 2023 when there are no longer reserve requirements for depositor banks and institutions at the Federal Reserva.  The response of the FFR to dummies representing inividual dates is very small compared to Bertolini el al:
...
- rises 0.067 bp rather falls at the end of the year
- rises  at end of quarters 1,2, and 3  by a much smaller 0.115 bp versus 2.08
...
DUring the period 2015-2023, esitmates show
- fall Friday, rise Monday -0.011, 0.005
