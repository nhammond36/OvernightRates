...
<!--
FOMC policy changes in the FFR changes the distribution of the reference rates. Former scarcity, reserve requirements OMO,
-->
...
library(tidyverse)
#spread_no_na$Three_mon_Tbill <- spread_no_na$m3tbillbp
#spread_no_na <- spread_no_na %>%
#+     mutate(Three_mon_Tbill = spread_no_na$m3tbillbp)
print(names(spread_no_na))
arbrates <- spread_no_na %>%
  select(sdate, EFFR, SOFR, IORR, RRPONTSYAWARD, Three_mon_Tbill)
# Three_mon_Tbill` doesn't exist.
arbrates$Three_mon_Tbill[is.na(arbrates$Three_mon_Tbill)] <- 0
# Ensure spread_no_na contains the updated m3tbillbp column
#spread_no_na <- spread_no_na %>%
# mutate(m3tbillbp = ifelse(is.na(m3tbillbp), 0, m3tbillbp))
```
