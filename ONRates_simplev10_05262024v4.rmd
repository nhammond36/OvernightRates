...
```{r, fig.caption= "Rates during normalcy 3/17/2016-7/31/2019",echo=FALSE,results='asis',warning=FALSE, message=FALSE}
library(ggplot2)
library(reshape2)
k=1
begn <- c(1, 859, 923, 1014, 1519, 1)
...
```{r, fig.caption="Rates during mid cycle adjustment 8/1/2019-10/31/2019", echo=FALSE, results='asis',warning=FALSE, message=FALSE}
k=2  
library(ggplot2)
library(reshape2)
begn <- c(1, 859, 923, 1014, 1519, 1)
...
```{r, fig.caption= "EFFR during adjustment period 8/1/2019-10/31/2019", rates_adjust,eval=TRUE, echo=FALSE, results='asis',warning=FALSE, message=FALSE}
library(ggplot2)                        
library(reshape2)
...
```{r, fig.caption= "Rates during covid 11/1/2019-3/16/2020",echo=FALSE,results='asis',warning=FALSE, message=FALSE}
library(ggplot2)
library(reshape2)
k=3  
library(ggplot2)
...
```{r, fig.caption= "EFFR during covid period 11/1/2019-3/16/2020",rates_covid,echo=FALSE,results='asis',warning=FALSE, message=FALSE}
library(ggplot2)
rates=2
...
```{r, fig.caption= "Rates during zero lower bound 3/17/2020-3/16/2022",echo=FALSE,results='asis',warning=FALSE, message=FALSE}
library(ggplot2)
library(reshape2)
k=4 
library(ggplot2)
...
```{r, fig.caption= "EFFR during zero lower bound (zlb) period 3/17/2020-3/16/2022",rates_zlb,echo=FALSE,warning=FALSE, message=FALSE}
library(ggplot2)
...
```{r, fig.caption= "Rates during Taming inflation 03/17/2022-12/14/2023",echo=FALSE,results='asis',warning=FALSE, message=FALSE}
library(ggplot2)
library(reshape2)
k=5
library(ggplot2)
...
```{r, fig.caption="EFFR during inflation period 3/17/2022-12/14/2023",rates_inflation,echo=FALSE,warning=FALSE, message=FALSE}
library(ggplot2)
library(reshape2)
...
Small interquartile range (IQR) values are evidence that the EFFR and the money market rates, with the exception of SOFR, cluster more tightly around their median values (Table \@ref(tab: IQR of rates during policy regimes). For the sample, 1.54 bp for the EFFR, and under one bp for the money market rates (middle panel Table). 
...
SOFR exhibits the greater variability; for the sample as a whole a high 4 bp and in all episodes. The dispersion of SOFR is even more diverse among policy regimes. SoFR rates have the largest spread around the median rate, reaching over 10 bp during the adjustment period. SOFR is more tightly clustered around its median during normaly, 3.30 bp and the zero lower bound  3.05 bp, but 7.37 bp during covid and 5.68 bp during inflation
...
\emph(Volume)
Daily volume in wholesale  rates TBGR  \$296.95 billion,and BGCR \$311.23 billion, almost quadruple the transactions in the EFFR: \$78.91 billion. SOFR transactions, \$754 billion per day, dwarf trading in all other overnight reference rates.
