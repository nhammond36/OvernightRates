...
# Introduction
Overnight reference rates, the whole money market rates, and the Federal Reserve's policy and adminsitrative rate share these features: clustering around the Federal Funds rate (FFR), quick response to changes in FFR and administrative rates, interest on reserves (IOR), the overnight reverse repo rate (ONRRP), large outliers, and heteroskedastcity, volatility that changes over time.
...
Before the financial crisis of 2008, policy was typically based on adjusting the scarcity value of a limited supply of central bank deposits (reserves). But the response of substantial asset purchases to the financial crisis of 2008 increased in liquidity, leaving the Fed to seek new ways  to manage the FFR. The subsequent experiments with different policy regimes occurent even as events that occurred during the six year period of our sample 2016-2023. Asset purchases under QE,QE1, QE2, and QE3, challenged the bank’s ability to control short-term interest rates. 
...
In this  corridor system, the interest on reserves (IOR) is the ceiling, the overnight reverse repo purchase rare (ONRRP) the floor.The ability to set the IOR and the ONRRP are key for the Fed’s implementation of monetary policy.In the ample reserves regime. the administered rates constitute a “floor” system to manages the FFR within its target range, the upper and lower targets set by the Federal Open Market Committee (FOMC). The interest on reserves rate (IOR or IORB) is the Fed’s principal tool for interest rate control.the Fed adjusts administrative rates to manages the FFR between the FOMC target upper and lower bounds.The IOR.  Banks have little or no incentive to lend their reserves to other banks at rates lower than the IOR rate they can receive from the Fed. The IOR is, in effect, a reservation rate for banks’ intermediation activities.
...
The administered rates, the IOR and the ON RPP,  along with an ample supply of reserves, created a corridor system for managing the Fed Funds rate. The IOR is the ceiling, the ON RRP the floor. Banks have little or no incentive to lend their reserves to other banks at rates lower than the IOR rate.No bank would lend at rates on overnight cash lower than the IOR, the rate they can receive from the Fed. The IOR rate serves as a reservation rate for banks’ intermediation activities,
Similarly, no bank should be borrow at a higher rate than the ONRRP. Banks and non-bank financial institutions should be unwilling to invest funds in private markets below the ONRRP. No bank should lend at a lower rate than the ONRPP. Competition among borrowers should drive rates to the discount window (DW) rate. 
...
Changes in the Fed funds rate and the administered rates IOR and the reward rate ON RRP are one source of shocks to overnight reference rates (\@ref{table:FOMC rate changes 2016-2023}) 
...
# Create a data frame with the notes
notes <- data.frame(
  Date = c("June 2020", "December 2020", "November 2021", "December 2021", 
           "March 3, 2020", "March 11, 2020", "March 15, 2020", "March 16, 2020", 
           "April 2020", "August 28, 2020", "July 28, 2021", "January 26, 2022", 
           "February 24, 2022", "March 16, 2022", "May 5, 2022", "June 2022", 
           "June 16, 2022", "July 28, 2022", "September 22, 2022", "November 3, 2022", 
           "December 15, 2022", "February 2, 2023", "March 23, 2023", "May 4, 2023", 
           "July 27, 2023"),
  Event = c("Fed set rate of purchases",
...
notes$Date <- as.Date(notes$Date, format = "%B %Y")

# Output the data frame as a table
#xtable_notes <- xtable(notes, caption = "Important Events Timeline", align = c("l", "p{10cm}"))
#print(xtable_notes)
```
...
The focus of this examination of overnight reference rates from 2016 through 2023 is the notable behavior of the wholesale credit market rate, the median Tri-Party General Collateral Rate (TGCR), the Broad General Collateral Rate (BGCR), and Secured Overnight Financing Rate (SOFR) to \@ref(fig:Sample rates 3/4/2016-12/14/2023)
- track and cluster around the policy rate, the effective Federal rate (EFFR)
- the tendency of all rates to respond quickly to changes in the EFFR

In addition, these overnight reference rates share known qualities  of short rates 
- the changing volatility or heteroskedasticity and 
 - outliers
...
The the average median rates and percentiles illustrate both the tendency of other rates to cluster around the EFFR and the presence of outliers.

The median, a measure of central tendency, and quantiles locate the clustering and range of values in the data. The average median EFFR over the full sample is 157 basis point (bp).  The money market rates about 132-134 bp, TGCR  132  bp,  BGCR 133 bp, SOFR 134 bp.  

Money market rates cluster around the median EFFR.The interquartile range (IQR), the 75th percentile minus the 25th percentile, a measure of variability for skewed distributions, captures where 50 percent of the data lie. The IQR shows how clustered are these reference rates. Fifty percent of EFFR daily rates fall within 157.71-156.15 bp, an ICR of  1.56 basis points (\@ref(tab:Sample statistics 2016-2023), the TGCR and BGCR cluster even more tightly, under one bp (133), SOHR  4.2 (136.70-132.50).S maller values if the IQR show that the middle values cluster more tightly.  Based on the middle half of the distribution, The IQR is less influenced by extreme values. 

Outliers are described by the range of the data.  A common measure of variability, a large range means high variability, a small range means low variability. The range, the 99th percentile minus the first percentile, is more sensitive to outliers. The range represents 2 percent of the data, the spread of the data from the lowest to the highest value in the distribution. The magnitude of outliers in all rates 1s 14 to 16 bp.  

??The most extreme values  are 23.36, 13.86, 16.11, and 14.51 basis point for the wholesale rates TGCR, BGCR, and SOFR.  TGCR and BGCR 132.83 and 132.84, SOFR 134.08. [range from 123 to 134 bp. 


The EFFR is on average within the FOMC target range, 145.52 to 170.52. [WHERE?]

The volume of SOFR transactions, $\$$754 billion per day, dwarf trading in all other overnight reference rates. with SOFR leading at approximately $\$$754 billion, followed by TGCR and BGCR at around $\$$300 billion each. EFFR has a notably lower daily trade volume of $\$$79 billion. 
...
Times series of rates during different policy episodes portray how the distribution of rates changes with monetary policy (Figure).  The median rates illustrate  how the central tendency changes with policy regime (Table). 

The average weighted median for the EFFR during the five episodes normalcy, adjustment, covid, zero lower bound, and inflation, illustrate the changing distribution of rates under different policies (\@ref(tab:Median rates of policy regimes). During normalcy, the median EFFR was 133.78 bp, adjustment 200.09 bp, covid 150.46 bp, zero lower bound 8.08 bp, and inflation 368.60 bp.all substantially different from the sample median EFFR 157 bp, 

Medians indicate the change of policy but are not very informative of the series. The median EFFR during normalcy, 133.79 bp, steadily increasing from 25 bp o 237 bp. The median rate during the adjustment period, 200.09 bp while rates from 258 bp to 158 bp.  During covid, the median rate is 150.46. Rates  remain at this level then plunge to zero.  Rates during the zero lower bound  where the mideian EFFR is 8.08 bp, are low,between zero and 20 bp, but are volatile.  During inflation, where the median EFFR is 533 bp,  rates rise steady in 25 and 50 bp increments from 25 to 533 bp. 
...
See (\@ref(tab:boxplots of rates during policy regimes) provides a summary of the data.

```{r, tab.cap="boxplots of rates during policy regimes", echo=FALSE, results='asis'}
library(patchwork)
# Combine boxplots
...
```
The IQR and range provide more information about the series, clustering and extreme values (\@ref(tab:IQR of rates during policy regimes)). . As in the sample (), the TGCR, BGCR and SOFR cluster around the EFFR except during zero lower bound periods. The wholesale  rates TGCR and BGCR rates cluster more tightly within 50 percent of the data than the EFFR or SOFR. 1 bp in the sample, 0.22 bp during normalcy; 1.3-1.4 bp during covid, between 0.12 and 0.15 during the the zero lower bound (\@ref(tab: IQR of rates during policy regimes). The largest IQR is during the inflation regime, 3.3-3.4 bp, but still small. SoFR rates have the largest spread within 50 percent of the data, reaching 10 bp during the adjustment period (\@ref(tab:boxplots of rates during policy regimes).

Larger values indicate that the central portion of your data are further spread out. Conversely, smaller values show that the middle values cluster more tightly. 
...
The range tells us about the outliers under different policies (Table \@ref(tab: Range of rates during policy regimes). Outliers during the covid and zero lower bound are within or less than the range of values in the sample. The magnitude of outliers in the sample 1s 14 to 16 bp.The magnitude of outliers is greatest duing the Fed's attemp to contain inflation.

A large range means high variability, a small range means low variability.The magnitude of outliers in all rates 1s 14 to 16 bp.  Adjustment, covid, and inflation episodes show higher variablity of rates compared to the same
