...
h <- data.frame(matrix(0, nrow = N, ncol = 7))
...
# Assuming 'holidays' is a vector of holiday dates------------------------------
library(timeDate)
...
# Generate a sequence of dates from January 1st of start_year to December 31st of end_year
date_sequence <- as.timeDate(seq(as.Date(paste(start_year, "-01-01", sep = "")),
                                 as.Date(paste(end_year, "-12-31", sep = "")), by = "days"))


# Convert date_sequence to character for matching
date_sequence_char <- as.character(date_sequence)

# Identify dates that are NYSE holidays
is_nyse_holiday <- date_sequence_char %in% as.character(nyse_holidays)

# Print the holiday dates
print(date_sequence[is_nyse_holiday])
#print(nyse_holidays)

# Assuming 'date_column_name' is the name of your date column
h$is_preceding_holiday <- c(FALSE, !is.element(h$date_column_name[-nrow(h)], holidays) & is.element(h$date_column_name[-1], holidays))

h$is_following_holiday <- c(!is.element(h$date_column_name[-1], holidays) & is.element(h$date_column_name[-nrow(h)], holidays), FALSE)

library(lubridate)
# Assuming 'date_column_name' is the name of your date column
h$is_last_day_of_quarter <- c(FALSE, quarters(h$date_column_name[-1]) != quarters(h$date_column_name[-nrow(h)]))

h$is_last_day_of_year <- c(FALSE, year(h$date_column_name[-1]) != year(h$date_column_name[-nrow(h)]))

# Assign value 1 to every observation where the day of the week is Friday
monday_column <- 1
friday_column <- 1
endquarter_column <- 1
endyear_column <- 1
...
# Assign value 1 to every observation where the day of the week is Friday
for (i in 1:N) {
  # Use Sys.Date() to get the current date
  current_date <- Sys.Date() + i - 1
  
  # Check if the day of the week is Friday (using weekdays())
