...
# > 
#   > str(quantilesE_long)
# 'data.frame':	13688 obs. of  3 variables:
#   $ sdate: Date, format: "2016-03-04" "2016-03-07" "2016-03-08" "2016-03-09" ...
# $ Rate : chr  "EFFR" "EFFR" "EFFR" "EFFR" ...
# $ Value: num  37 37 37 37 37 37 37 37 37 37 ...
# I apologize for any confusion. It seems I misunderstood your question. The result you're seeing is expected when using the gather function from the tidyr package.
# 
# In the gathered data frame (quantilesE_long), the original columns other than sdate are combined into two new columns: Rate (containing the original column names) and Value (containing the values from those columns). This is why you see only three columns in the resulting data frame.
# 
# The gather function takes the columns specified in the -sdate argument, collapses them into two columns (Rate and Value), and maintains the original sdate column.
# 
# To be clear, the quantilesE_long data frame has three columns: sdate, Rate, and Value. The Rate column contains the names of the original columns, and the Value column contains the corresponding values. This reshaping is useful for creating a tidy, long-format data frame suitable for various analyses and visualizations.
...
# https://cran.r-project.org/web/packages/tidyr/vignettes/pivot.html
 ...
 quantileseffr <- ggplot(melteffr,aes(x=sdate,y=value,colour=variable,group=variable)) + 
   geom_line() +
   labs( y = "Basis Points (bp)", color = "Rate", linetype = "Rate") +  
   scale_y_continuous(breaks = seq(0, 500, by = 50), limits = c(0, 500)) + 
   theme_minimal()+
