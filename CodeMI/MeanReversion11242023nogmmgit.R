...
# Still need this?  Check if used with last plota
rrbp_long <- rrbp_long %>%
   group_by(sdate, Rate) %>%
...
y_limits=500
 ...
 # any(is.na(quantilesE_long$Value))
 # [1] FALSE
 # unique(quantilesE_long$Rate)
 # [1] "EFFR"          "VolumeEFFR"    "TargetUe"      "TargetDe"      "PercentileE1"  "PercentileE25"
 # [7] "PercentileE75" "PercentileE99"
 # >  nrow(quantilesE_long)
 # [1] 13688
 # 
 ...
 # https://stackoverflow.com/questions/45966417/how-to-specify-multiple-columns-with-gather-function-to-tidy-data
 # df %>% gather(key = 'Age.group', value = 'Value.name', X0tot4:X20tot24)
 # f %>% gather(key = "Age.group", value = value, -Country, -Country.Code, -Year)
...
# https://cran.r-project.org/web/packages/tidyr/vignettes/pivot.html
 ...
 #quantilesE$VolumeEFFR= spread_no_na$VolumeEFFR
 #str(quantilesE)
 quantilesE2 = subset( quantilesE, select = -c(VolumeEFFR, PercentileE1, PercentileE99) ) 
 library(ggplot2)
 library(reshape2)
 melteffr <- melt(quantilesE2,id="sdate")
 quantileseffr <- ggplot(melteffr,aes(x=sdate,y=value,colour=variable,group=variable)) + geom_line() + theme_minimal()
 quantileseffr <- quantileseffr + theme(legend.title = element_blank())
 print(quantileseffr)
 ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/quantileseffrline.pdf")
 ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/quantileseffrline.png")
 
 #quantilesS$VolumeSOFR= spread_no_na$VolumeSOFR
 #quantilesS$sTargetU[120]=50
 #str(quantilesS)
 quantilesS2 = subset( quantilesS, select = -c(VolumeSOFR, PercentileS1, PercentileS99) ) 
 meltsofr <- melt(quantilesS2,id="sdate")
 quantilessofr <- ggplot(meltsofr,aes(x=sdate,y=value,colour=variable,group=variable)) + geom_line() + theme_minimal()
...
#FF > upper target TU
# Equation 1:
D_t <- ifelse(rho_max_t < bar_rho_t, bar_rho_t - rho_max_t, 0)

# Equation 2:
D_t <- ifelse(bar_rho_t < rho_min_t, bar_rho_t - rho_min_t, 0)

# Equation 3:
D_t <- ifelse(bar_rho_t < rho_min_t, bar_rho_t - rho_min_t, 0)

# Equation 4:
D_t <- ifelse(rho_min_t < bar_rho_t & bar_rho_t < rho_max_t, 0, 0)

# Create a data frame with the equations
equations_df <- data.frame(Equation = c("D_t <- ifelse(rho_max_t < bar_rho_t, bar_rho_t - rho_max_t, 0)",
                                        "D_t <- ifelse(bar_rho_t < rho_min_t, bar_rho_t - rho_min_t, 0)",
                                        "D_t <- ifelse(bar_rho_t < rho_min_t, bar_rho_t - rho_min_t, 0)",
                                        "D_t <- ifelse(rho_min_t < bar_rho_t & bar_rho_t < rho_max_t, 0, 0)"))
...
# Display the table
kable(equations_df, "markdown", col.names = "Equation")
...
kable(equations_df, format = "latex", escape = FALSE, booktab
      ...
      # Make sure to use <- or = for assignments.
      D_t <- 0
      rho_bar_t <- 0
      rho_max_t <- 0
      rho_min_t <- 0
...
meltnorm <- melt(norm2,id="sdate")
normrates <- ggplot(meltnorm,aes(x=sdate,y=value,colour=variable,group=variable)) + geom_line() +
  labs(x = "Date", y = "Basis Points (bp)", caption="Rates during normalcy 3/4/2016-7/31/2019", color = "Rate", lineype = "Rate") + 
  theme_minimal()
normrates <- normrates + theme(legend.title = element_blank())
print(normrates)
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/normratesmelt.pdf")
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/normratesmelt.png")
...
meltadjust <- melt(adjust2,id="sdate")
adjustrates <- ggplot(meltadjust,aes(x=sdate,y=value,colour=variable,group=variable)) + geom_line() +
  labs(x = "Date", y = "Basis Points (bp)", caption="Rates during adjustment 8/1/2019-10/31/2019", color = "Rate", lineype = "Rate") + 
  theme_minimal()
adjustrates <- adjustrates + theme(legend.title = element_blank())
print(adjustrates)
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/adjustratesmelt.pdf")
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/adjustratesmelt.png")
...
meltcovid <- melt(covid2,id="sdate")
covidrates <- ggplot(meltcovid,aes(x=sdate,y=value,colour=variable,group=variable)) + geom_line() +
  labs(x = "Date", y = "Basis Points (bp)", caption="Rates during covid 11/1/2019-3/16/2020", color = "Rate", lineype = "Rate") + 
  theme_minimal()
covidrates <- covidrates + theme(legend.title = element_blank())
print(covidrates)
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/covidratesmelt.pdf")
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/covidratesmelt.png")
...
meltzlb <- melt(zlb2,id="sdate")
zlbrates <- ggplot(meltzlb,aes(x=sdate,y=value,colour=variable,group=variable)) + geom_line() +
  labs(x = "Date", y = "Basis Points (bp)", caption="Rates during zero lower bound 3/17/2020-3/16/2022", color = "Rate", lineype = "Rate") + 
  theme_minimal()
zlbrates <- zlbrates + theme(legend.title = element_blank())
print(zlbrates)
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/zlbratesmelt.pdf")
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/zlbratesmelt.png")
...
meltinflation <- melt(inflation2,id="sdate")
inflationrates <- ggplot(meltinflation,aes(x=sdate,y=value,colour=variable,group=variable)) + geom_line() +
  labs(x = "Date", y = "Basis Points (bp)", caption="Rates during inflation 03/17/2022-12/29/2022", color = "Rate", lineype = "Rate") + 
  theme_minimal()
inflationrates <- inflationrates + theme(legend.title = element_blank())
print(inflationrates)
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/inflationratesmelt.pdf")
ggsave("C:/Users/Owner/Documents/Research/MonetaryPolicy/Figures/Figures2/inflationratesmelt.png")
# ----------------------------------
...
#norm_sdate <- sdate[1:856]
ggnorm <- ggplot(norm, aes(x = norm_sdate)) +
