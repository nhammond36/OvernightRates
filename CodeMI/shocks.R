...
#fed_no_na$Date <- as.Date(fed_no_na$Date, format = "%m/%d/%Y")   Why?  akready done sdate

#Define a calendar with US holidays and weekdays
us_holidays <- as.Date(c("2016-01-01", "2016-07-04", "2016-12-25"))
cal <- create.calendar(name = "US", holidays = us_holidays, weekdays = c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday"))

# Select all columns except the first three, then check if the date is a business day
fed_no_na <- fed_no_na %>%
  select((2:4)) %>%
  #select(-(4:6)) %>%
  filter(isBizday(sdate, cal))

# Display the modified dataframe
print(fed_no_na)

fedtemp<-c(fed_no_na$Date,fed_no_na$DFEDTARL,fed_no_na$DFEDTARU,fed_no_na$IORR)
str(fedtemp)
sdate

# ----------------- This worked but doesnt eliminate correct dates
library(bizdays)
#us_holidays <- as.Date(c("2016-01-01", "2016-07-04", "2016-12-25"))
us_holidays <- as.Date(c("2016-01-01", "2016-07-04", "2016-12-25"))
cal <- create.calendar(name = "US", holidays = us_holidays, weekdays = c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday"))
...
# Assuming fed_no_na$Date is in the format "%m/%d/%Y"
fed_no_na$Date <- as.Date(fed_no_na$Date, format = "YYYY-MM-DD")
#-------------------------
# Create a calendar with US holidays and weekdays
us_holidays <- as.Date(c("2016-01-01", "2016-07-04", "2016-12-25"))
cal <- create.calendar(name = "US", holidays = us_holidays, weekdays = c("monday", "tuesday", "wednesday", "thursday", "friday"))

# Define a function to check if a date is a business day
isBizday <- function(dates, cal) {
  # Check if the dates are weekends
  is_weekend <- format(dates, "%u") %in% c("6", "7")

# Check if the dates are holidays
is_holiday <- dates %in% as.Date(us_holidays)

# Return TRUE for dates that are neither weekends nor holidays
return(!is_weekend & !is_holiday)
}

# Filter rows based on business day for columns 1:3
filtered_rows <- fed_no_na %>%
  filter(isBizday(Date, cal))
...
# ----
fed_no_na <- fed_no_na %>%
  select(fed_no_na$DFEDTARL, fed_no_na$DFEDTARU, fed_no_na$IORR) %>%
  filter(isBizday(sdate, cal))
...
# This checks for rows not filtered -----------------
# Identify dates that are not being filtered correctly
not_filtered_dates <- fed_no_na %>%
  filter(!isBizday(Date, cal)) %>%
  select(Date, Date_char)
