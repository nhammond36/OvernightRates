...
# alterntaive moments IQR and MAD
#For a symmetric distribution (where the median equals the midhinge)
# thenn median equals the average of the first and third quartiles)
# half the IQR (QR = Q3 −  Q1) equals the median absolute deviation (MAD).
# medeffr =(107.46639+109.32320)/2
# > medeffr
# [1] 108.3948
# > iqreffr=  109.32320- 107.46639 
# > madeffr=iqreffr/2
# > print( medeffr)
# [1] 108.3948
# > print(iqreffr)
...
# Volattility and dispersion measures
# 
# NYFed
# The deviation of the effective funds rate
...
# Define vectors for 'target' and 'drates'
target <- matrix(0, nrow = n, ncol = 2)  # Replace 'n' with the actual number of rows
drates <- matrix(0, nrow = n, ncol = 1)  # Replace 'n' with the actual number of rows
...
for (t in seq_len(n)) {
  if (target[t, 2] < rrbp[t, 1]) {
    # Upper target
    g[t] <- rrbp[t, 1] - spread$TargetUe[t]
  } else if (rrbp[t, 1] < spread$TargetDe[t]) {
    # Lower target
...
gara <- ggplot(rrbp, aes(x = sdate)) +
  geom_line(aes(y = rrbp[begn(k):endn(k),1], color = "EFFR"), linewidth =1) + 
  geom_line(aes(y = TargetDe[begn(k):endn(k),2], color = "Lower target"), linewidth = 1) + 
  geom_line(aes(y = TargetUe[begn(k):endn(k),3], color = "Upper target"), linewidth = 1) + 
  ...
  scale_color_manual(values = c("EFFR" = "black", "Lower target rate" = "blue", "Upper target rate" = "green", "MAD" = "orange", "Gara" = "red")) + 
