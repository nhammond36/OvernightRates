...
spec6 <- ugarchspec(
  ...
  distribution.model = "std"  # Specify GED distribution
)
...
pec_ged <- ugarchspec(
  ...
  distribution.model = "ged",  # Specify GED distribution
  distribution.p = 2  # Set the degrees of freedom for GED
)

fit_ged <- ugarchfit(spec_ged, data = rrbp[, 2], solver.control = list(tol = 1e-6, maxit = 2000))
...
spec_ged <- ugarchspec(
  variance.model = list(
    model = "sGARCH",  # Use sGARCH instead of eGARCH
    external.regressors = edata,
    garchOrder = c(1, 1)
  ),
  distribution.model = "ged",  # Specify GED distribution
...
egarch.fit = ugarchfit(egarch11.spec,Return.BMW)
egarch.fit
...
#fit EGARCH model
egarch.fit.t=ugarchfit(data=Return.BMW,spec=spec)
...
#fit EGARCH model
egarch.fit.t=ugarchfit(data = return.effr,external.data=matrix(ext_effr),spec=spec_effr)
