...
```{r sampleplot_example, fig.cap="Overnight rates 2016-2023", echo=FALSE}
...
```{r sampletable_example, tab.cap="Rate characteristics 2016-2023", echo=FALSE, results='asis'}
...
print(stats)
```
As shown in Figure \@ref(fig:sampleplot_example), the relationship is evident. Table \@ref(tab:sampletable_example) provides a summary of the data.
...
```{r ratestats_sample, tab.cap="Rate statistics 2016-2023", echo=FALSE, results='asis'}
...
```
\@ref(tab:ratestats_sample) provides simple statistics of the data.
...
quantilesEnum <- quantilesE[1:nrow(quantilesE), 2:8]
...
```{r tab1, tab.cap="Sample statistics forovernight rates 2016-2023"}
# Sample data in two lists
categories = c('EFFR', 'OBFR', 'TGCR', 'BBGCR', 'SOFR')
median_values = c(Estats[1], Ostats[1], Tstats[1], Bstats[1], Sstats[1])
...
<!--
plot table references example
```{r fig1, fig.cap="Caption for Figure 1"}
plot(1:10)
```
Figure \@ref(fig:fig1) shows..

```{r tab1, tab.cap="Caption for Table 1"}
head(mtcars)
```
Table \@ref(tab:tab1) shows...
-->
...
Volatily in the sample, the standard deviation of log percent change
...
[CORRECT EQUATION]
 ...
 \begin{align*}
 nrates<-ncol(rrbp)
 D_t =\frac{1}(sum_{i=1}^{nrates}{v_{i,t}) X abs{\hat{y_{i,t} − \bar{y_t}) (4.2)
