...
# char_inflation<-colMeans(quantilesE[bgn_edn,]     #inflation
Estatsinflation <- colMeans(qinflationE[,2:ncol(qinflationE)], na.rm = TRUE)
#Ostatsinflation <- colMeans(qinflationO[,2:ncol(qinflationO)], na.rm = TRUE)
Tstatsinflation <- colMeans(qinflationT[,2:ncol(qinflationT)], na.rm = TRUE)
Bstatsinflation <- colMeans(qinflationB[,2:ncol(qinflationB)], na.rm = TRUE)
...
```{r combinedtable_characteristics} 
# Define variable names
...
[MISSING INFLATION PLOT]
(see Figure \@ref(fig:EFFR during inflation period 3/17/2020-3/16/2022
```{r, fig.cap= "EFFR during inflation period 3/17/2022-12/14/2023"}
sdate<-sdateinflation
maxr=max(inflation[,2:ncol(inflation)]) # 539
minr=min(inflation[,2:ncol(inflation)]) #0
...
(see Figure \@ref(fig:Rates during different polic episodes)
```{r, fig.cap= "Rates during different polic episodes"}
...
str(dkindexdf)
```
(see Figure \@ref{fig:Duffie Krishnamurthy index})
