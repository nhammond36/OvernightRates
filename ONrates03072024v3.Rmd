...
my_envmp <- readRDS("C:/Users/Owner/Documents/Research/OvernightRates/my_envmp.RDS")
```
```{r, load environment effr, echo=FALSE}
my_enveffr <- readRDS("C:/Users/Owner/Documents/Research/OvernightRates/my_environmenteffr.RDS")
```
```{r, load environment sofr, echo=FALSE}
my_envsofr <- readRDS("C:/Users/Owner/Documents/Research/OvernightRates/my_environmentsofr.RDS")
```
```{r, load environment episodes, echo=FALSE}
...
vold <- data.frame(
  subset(my_data, select = c("VolumeEFFR", "VolumeOBFR","VolumeTGCR","VolumeBGCR","VolumeSOFR"))
)
```
...
All authors recognize key characteristics of the FFR; Spectacular outliers and autoregressive conditional heteroskedasticity (ARCH) effects, skew, fat tails. 
...
where E∗ denotes expectation under Q∗. Standard results show that if there exists a riskneutral probability measure Q∗, a system of asset prices is arbitrage fre
...
where S =  t+τ t ru du (as above). The LEH therefore amounts to risk-neutral pricing: the datagenerating measure Q and the risk-neutral measure Q∗ coincide. This means that
...
Affine Term Structure Models S is also Gaussian (as the sum of Gaussians). With this normality assumption, Eq. (2.4) becomes   $y^{\tau}_t= E_t[S] /\tau− .5 var_t[S] /\tau$
...
Piazzesi The “snake shaped ”volatility of yields:Volatility is high
for very short maturities (the head of the snake), rapidly decreases until
maturities of around three months (the neck of the snake), then increases until maturities of up to two years (the back of the snake), and finally decreases again (its tail).

?? which the model explains with policy inertia, tendency to continue same policy changes
...
Hamilton, as other authors, recognizes extreme volatility and the large outliers of short-rate data. Hamilton provides evidence of  day effects, as publically available bank balance sheets  provide a snapshot of assets and liabilities on the last day of the quarter or the last day of the year.
 
 Hamilton (1996) and others have concluded that the hypothesis that the FFR follows a martingale is inconsistent with observed daily changes in the federal funds rate. (Bertolini,Bertola, Prati ,) site frictions such as  transactions costs, interbank credit limits, overdraft penalties, obstacles to intra-period arbitrage preclude the FFR from following a martingale process. 

Benzoni identify mean reversion around a central tendency. The stochastic mean  allows a relatively fast mean-reversion of the short rate around a highly persistent time-varying central tendency process. Jumps are integral to the quality of fit and relieve the stochastic volatility factor from accommodating extreme outlier behavior. 

The mean drift may be indicative of slowly evolving inflationary expectations (Gara horiz?), time-variation in the required real interest rate, or both.

Heteroskedascity, outliers, volatility, fat tails, skew
...
Bertolini, Bertola, Prati () focus is the high frequency patterns of FF rate volatility  that survive Fed's attempt to manage the FFR within the FOMC target range. Rate data provide evidence of Fed's difference preferences for offsetting volatility in the Federal Funds market

They note the response of other key short rates to monetary policy and other shocks. Overnight wholesale money market rates quickly revert to changes in the unsecured federal funds rate (FFR) or administered rates. 

Their model of FFR volatility seeks to isolate Fed preferences for offsetting volatility in the FFR market.[describe model]
##
...
For the sample from 2016 through 2023, (Figure \@ref(fig:Daily rates 2016-2022-plot), all overnight rates closely track the EFFR. Fed policy changes visible in Vertical shifts. Rates increae steadily rates from _ basis points during normalcy (2016 to_ July 2019) 2016-2019, In 2019 rates began their decline in a 3 month period from August to October 2019. Rates then plunge with the onset of covid, from _ basis points in November 2019 to March 2020 during covid (November 2019 to March 2020, the   Rates plunge to near zero from Mar 2020 to March 022 during the zero lower bound. As the Fed began to fight inflation rates rose in steady 25 or 50 basis point increments from March 2022 to_ in December 2023. Policy inertia also evident in all periods, as the Fed sequentially changes rates in ?? basis points .


Rates were held steady from _ basis points in 2016 to_  

Figure 2
Volatility;m High, outliers, heteroskedastic, volatility survives rate management
Extreme or high volatility of short rates
Outliers
Rate volatility survives Fed management of FFR

Conditional heteroskedasticity (\url{https://www.investopedia.com/terms/h/heteroskedasticity.asp})
Heteroskedasticity often arises in two forms: conditional and unconditional. Conditional heteroskedasticity identifies nonconstant volatility related to prior period's (e.g., daily) volatility. 
...
The range 99-1 shows 98 percent of the data 
...
Since the data are average weighed median values, measures of dispersion. the spread around a central tendency, are the interquartile range - the 75th minus the 25 percentile, and the range - the 99th minus the one percentile). The interquartile range sumarizes the variability when outliers are present in the data. The range discribes outliers in a sample. \url{https://www.sciencedirect.com/topics/computer-science/interquartile-range}
...
```{r, sample numerica dataframes, echo=FALSE}
quantilesEnum <- quantilesE[1:nrow(quantilesE), 1:8]
...
Estats <- colMeans(quantilesEnum)
Ostats <- colMeans(quantilesOnum)
Tstats <- colMeans(quantilesTnum)
Bstats <- colMeans(quantilesBnum)
Sstats <- colMeans(quantilesSnum)
#, na.rm = TRUE
```

```{r, sample statsdata2, echo=FALSE}
...
quantilesS2 = subset( quantilesS, select = -c(VolumeSOFR,Percentile01_SOFR, Percentile99_SOFR) ) 
...
<!--
Figure 3
Martingale or not: Hamilton (1996) and others have concluded that the hypothesis that the FFR follows a martingale is inconsistent with observed daily changes in the federal funds rate
...
INSERT BOXPLOT  histograms, densities, boxplots -----------------------------------



```{r, daily sample quantile dataframes, echo=FALSE,  cache=FALSE,results='hide'}
...
bgn<-begn[k]
edn<-endn[k]
#norm<-rrbp[bgn:edn,]  # duplicate to fix since rates are in qnorm etc
qnormE=quantilesE[bgn:edn,]
Estatsnorm <- colMeans(quantilesEnum[bgn:edn,], na.rm = TRUE)
Ostatsnorm <- colMeans(quantilesOnum[bgn:edn,], na.rm = TRUE)
Tstatsnorm <- colMeans(quantilesTnum[bgn:edn,], na.rm = TRUE)
Bstatsnorm <- colMeans(quantilesBnum[bgn:edn,], na.rm = TRUE)
Sstatsnorm <- colMeans(quantilesSnum[bgn:edn,], na.rm = TRUE)
```
...
zlb-----------------------------------
                        EFFR VolumeEFFR Percentile01_EFFR Percentile25_EFFR Percentile75_EFFR
...
Estatsadj <- colMeans(quantilesEnum[bgn:edn,], na.rm = TRUE)
Ostatsadj <- colMeans(quantilesOnum[bgn:edn,], na.rm = TRUE)
Tstatsadj <- colMeans(quantilesTnum[bgn:edn,], na.rm = TRUE)
Bstatsadj <- colMeans(quantilesBnum[bgn:edn,], na.rm = TRUE)
Sstatsadj <- colMeans(quantilesSnum[bgn:edn,], na.rm = TRUE)
```


```{r, adjust characteristics,echo=FALSE}
selected_columnsE<- Estatsadj[c("EFFR","VolumeEFFR","Percentile01_EFFR", "Percentile25_EFFR","Percentile75_EFFR","Percentile99_EFFR")]
...
qcovidE=quantilesE[bgn:edn,]
Estatscovid <- colMeans(quantilesEnum[bgn:edn,], na.rm = TRUE)
Ostatscovid <- colMeans(quantilesOnum[bgn:edn,], na.rm = TRUE)
Tstatscovid <- colMeans(quantilesTnum[bgn:edn,], na.rm = TRUE)
Bstatscovid <- colMeans(quantilesBnum[bgn:edn,], na.rm = TRUE)
Sstatscovid <- colMeans(quantilesSnum[bgn:edn,], na.rm = TRUE)
```

```{r, covid characteristics,echo=FALSE}
selected_columnsE<- Estatscovid[c("EFFR","VolumeEFFR","Percentile01_EFFR", "Percentile25_EFFR","Percentile75_EFFR","Percentile99_EFFR")]
...
qzlbE=quantilesE[bgn:edn,]
Estatszlb <- colMeans(quantilesEnum[bgn:edn,], na.rm = TRUE)
Ostatszlb <- colMeans(quantilesOnum[bgn:edn,], na.rm = TRUE)
Tstatszlb <- colMeans(quantilesTnum[bgn:edn,], na.rm = TRUE)
Bstatszlb <- colMeans(quantilesBnum[bgn:edn,], na.rm = TRUE)
Sstatszlb <- colMeans(quantilesSnum[bgn:edn,], na.rm = TRUE)
```

```{r, zlb characteristics,echo=FALSE}
selected_columnsE<- Estatszlb[c("EFFR","VolumeEFFR","Percentile01_EFFR", "Percentile25_EFFR","Percentile75_EFFR","Percentile99_EFFR")]
 zlbchar <- rbind(selected_columnsE, Tstatszlb,Bstatszlb,Sstatszlb)
 zlbchar2 <- xtable(zlbchar)
 print(zlbchar2)
```

```{r, zlb stats,echo=FALSE
...
Estatsinflation <- colMeans(quantilesEnum[bgn:edn,], na.rm = TRUE)
Ostatsinflation <- colMeans(quantilesOnum[bgn:edn,], na.rm = TRUE)
Tstatsinflation <- colMeans(quantilesTnum[bgn:edn,], na.rm = TRUE)
Bstatsinflation <- colMeans(quantilesBnum[bgn:edn,], na.rm = TRUE)
Sstatsinflation <- colMeans(quantilesSnum[bgn:edn,], na.rm = TRUE)
```
```{r, inflation characteristics,echo=FALSE}
selected_columnsE<- Estatsinflation[c("EFFR","VolumeEFFR","Percentile01_EFFR", "Percentile25_EFFR","Percentile75_EFFR","Percentile99_EFFR")]
...
p1=8
p1=9
#my_episode<-my_envepisodes$norm[,rates]
my_episode<-qnormE[,rates]
```
...
p1=8
p1=9
#my_episode<-my_envepisodes$adjust[,rates]
my_episode<-qadjustE[,rates]
max(qadjustE[,rates]) #230
min(qadjustE[,rates]) #158
```
...
sdate <- as.Date(qadjustE[, 1])
data_without_sdate <- qadjustE[, -1]
...
p1=8
p1=9
#my_episode<-my_envepisodes$covid[,rates]
my_episode<-qcovidE[,rates]
...
p1=8
p1=9
#my_episode<-my_envepisodes$zlb[,rates]
my_episode<-qzlbE[,rates]
```
...
p1=8
p1=9
#my_episode<-my_envepisodes$inflation[,rates]
my_episode<-qinflationE[,rates]
```
...
qnormE2 = subset(qnormE, select = -c(VolumeEFFR, Percentile01_EFFR, Percentile99_EFFR) ) 
...
qadjustE2 = subset(qadjustE, select = -c(VolumeEFFR, Percentile01_EFFR, Percentile99_EFFR) ) 
...
str(box_data)
```
```{r, normalcy episode quantile dataframes, echo=FALSE,  cache=FALSE,results='hide'}
...
```{r, fig.cap= "Daily rates normalcy 2016-2019", echo=FALSE, warning=FALSE, message=FALSE, fig.height=6, fig.width=8, eval=TRUE, cache=FALSE, dev='png', out.width='80%', , results='hide', fig.pos='H'} 
...
#mxq = max(norm_measure1[,1:5])
#mnq = min(norm_measure1[,1:5])
...
```{r, plots sample log percent change, echo=FALSE}
...
Duffie and Krishnamurthy () and Gara Afonso, Kyungmin Kim, Antoine Martin, Ed Nosal, Simon Potter, and Sam Schulhofer-Wohl ()  study of dispersion of overnight wholesale money market rates from the policy rate that suggests problems for transmission of FONC policy to the broader economy.

Dispersion of rates imply rates follow different distributions. For the objective of this paper, to understand Fed tolerance for volatility of the FFR, we would need to examine any relation between volatility, the distance of the FFR within its target range. 
...
The Duffie and Krishnamurthy index of dispersion of overnight rate is the weighted mean absolute deviation of the cross-sectional adjusted rates.  Duffie, Krishnamurthi identify dispersion across money market interest rates as a primary indicator of the level of efficiency in how other key short-term interest rates reflect Fed policy changes. This paper examines the dispersion and volatility of short term rates. They argue that the current setting of U.S.-dollar money markets limits the transmission of Fed policy to the economy.
...
meanr<- colMeans(rrbp[,1:ncol])
# EFFR  OBFR  TGCR  BGCR  SOFR 
# FALSE FALSE FALSE FALSE FALSE 
colsumvold <- rowSums(vold[, 1:ncol], na.rm = TRUE)
  T<-nrow(rrbp)
  # Calculate dk(t)
  for (t in 1:T) {
  for (i in 1:ncol) {
...
#dk_data <- cbind(sdate, dk, dkindex,TargetDe, TargetUe, EFFR, OBFR, TGCR, BGCR, SOFR )
...
#dkindex <- data.frame(dk_data)
# Assign names to the columns of 'dk' within 'dk_data'
#as.Date(dkindex$sdate, format = "%y/%m/%d)
```
...
# Initialize matrices and vectors to store results
dk <- matrix(0, nrow = T, ncol = ncol)
dkindex <- numeric(T)
...
$v_{i,t} is the estimated outstanding amount of this instrument on day t (dollars).
 
 \begin{align*}
 D_t =1/ (sum_{i}^{v_{i,t}) X (sum_{i}^{}v_{i,t} abs{\hat{y_{i,t} − \bar{y_t}) (4.2)
 \bar{y_t} = [(sum_{i}^{n}(v_{i,t}) X \hat{y_{i,t})]/(sum_{i}^{v_{i,t})
 \end{align*}
 <!--
  correct eqn
\begin{equation*}
...
The EFFR stays within the bounds of the FOMC target rates (left panel of Figure ~\ref{igure:}) and the 35th and 75th percentile (right panel)


The SOFR stays within the bounds of the FOMC target rates (left panel of Figure ~\ref{igure:}) and the 35th and 75th percentile (right panel)


gara_data <- cbind(sdate, g, TargetDe, TargetUe, EFFR)
# Convert the numeric matrix 'dk_data' to a data frame
garaindex <- data.frame(gara_data)
...
Section 4 semi-nonparametric (SNP) model selection procedure used to obtain the moment conditions for the subsequent EMM estimation step. The family of SNP densities introduced
...
score \url{https://scikit-learn.org/stable/modules/model_evaluation.html}
...
A. Model
The state vector $Xt_[\theta,s,v,z]$  is , where $\theta$ is the federal funds target, 
s is the spread between the short rate r  and the target $\theta$, v is the
volatility of s, and z captures other macroeconomic information the Fed
uses in setting the target. All variables except v are unobservable but
can be inferred from yields through the bond-pricing model. The dynamics of the state variables are

\begin{align*}
d\theta_t = 0.0025(dN^U-dN^D ), (1)
ds_t = \kappa_s sdt + \sqrt{v_t}dw^s_t , (2)
dv_t = \kappa_v (\bar{v} - v )dt + \sigma_v \sqrt{v_t}dw^v_t , (3)
dz_t = -\kappa_z dt ^ dw^z_t, (4)
\end{align*}


where $N^U and dN^D$ and are counting processes with stochastic intensities $\lambda_U and \lambda_D$  
, respectively, and $w^$, are independent Brownian motions
IV. Estimation
The FOMC targets the federal funds rate, which pertai ns to interbank loans. These loans are not default-free because they are not collateralized. As a result, the federal funds rate and its target are substantially higher than short Treasury-bill rates (which are further depressed by
tax and liquidity effects). To estimate the model, I therefore use rates on LIBOR and swap contracts, which are traded mainly between banks.
The time t swap rate is the fixed rate at which banks can borrow for t years in exchange for floating payments that have a discounted present value of $1. The contract specifies that both the loan and the floating repayment be paid in biannual installments. The time t swap rate $Y(t,t+\tau)$ is then determined as the rate that equalizes the present discounted value of these installments at time t:

\begin{align*}
1= P(t,t+\tau) .5 Y(t,t+\tau) \sum_{j=1}^{2\tau}P(t,t+0.5j), (14)
\end{align*}

where the left-hand side is the $1 worth of floating repayments and the
right-hand side is the value of the biannual fixed loan payments (which
explains the division by two). Following Duffie and Singleton (1997), I
interpret the symbol r as the rate on short bonds of LIBOR and swap
quality. This means that r reflects the credit risk of interbank loans, just
like the Fed’s target.


The parameter vector $\gamme$contains 14 parameters for the intensities, $\abs{\lambda},\lambda_{\theta},\lambda_s,\lambda_v,\lambda_z$; the mean reversions $\kappa_s,\kappa_v,\kappa_z$ and ; the means $\bar{theta} and \bar{nu}; the volatility $\sigma_{\nu}$; and the risk premia $q_s,q_v , andq_z$

For a given parameter $\gamma$ sv z qq q
vector g, the model maps the state vector $X_t$ into observables $Y_t$ based on equation (14). 

The vector of observables contains the target, the six-month LIBOR, and the two- and five-year swap yields:
$Y_t= [\theta  Y(t, t+0.5) Y(t, t+2) Y(t, t+5)]^\tau $

Section IV.A describes the data on . Yt
Ideally, the parameters would be estimated by maximizing the likelihood function of the observables over g. The likelihood function is
the product of densities conditional on the last obser- f(Yt
, tFYt, t; g)
vation at some . The density Y t ! t f can be obtained by a change of t
variable from the conditional density of : fX(Xt
, tFXt, t; g) Xt
f(Yt
, tFYt, t; g) p fX(g(Yt
, g), tFg(Yt, g), t; g)F∇Yg(Yt
, g)F, (15)
where is the function from the observables to the state vector, g(7, g)
in that . This function inverts the yield formulas (14). Xt t p g(Y , g)
Now, three problems arise. First, the true density is not available fX
in closed form. I therefore extend the simulated maximum likelihood
(SML) method of Pedersen (1995) and Santa-Clara (1995) to jump
diffusions (Sec. IV.B). Second, the function needs to be inverted g(Yt
, g)
numerically for each observation t. To do this, I use a hill-climbing
method based on analytical gradients. As a by-product, I get the Jacobian
term analytically (Sec. IV. F∇ g(Y , g)F C). Finally, the function g does not Y t
impose that intensities and the target need to be positive. 
Benzoni----------------------------------------------
three-factor jump-diffusion model, a time-varying mean reversion factor, a stochastic volatility factor, and a jump process, capture main features of the U.S. short-term interest rate 3 month Tbill.  The U.S. short-term interest rate is characterized by complex conditional heteroskedasticity, fat-tailed innovations, and pronounced autocorrelation patterns.   Stochastic volatility is critical for a good fit. The stochastic mean  allows a relatively fast mean-reversion of the short rate around a highly persistent time-varying central tendency process. Jumps are integral to the quality of fit and relieve the stochastic volatility factor from accommodating extreme outlier behavior. 

The mean drift may be indicative of slowly evolving inflationary expectations (Gara horiz?), time-variation in the required real interest rate, or both.

 the term structure literature has relied predominantly on models in the so-called
affine class; see, e.g., Duffie and Kan (1996)

\begin{equation}
dr_t=\kappa_1(\mu_t-\frac{\mu_J \lamba}{kappa_1}-r_t)dt +\sqrt{V,t}dW_{1,t}+Z_tdq_t  (8)
dV_t=\kappa_2(\alpha-\mu_t)dt +\eta_1\sqrt{V_t}dW_{2,t}                              (9)
d\mu_t=\kappa_3(\nu-\mu_J)dt +\eta_3\sqrt{\mu_t}dW_{3,t}                            (10)
\end{equation}

where $W_1-W_3} and q satisfy the same assumptions as those stated for (1)-(3), except that here we
assume $corr(dW_{1,t}, dW_{i,t}$) = 0, i = 2, 3. 

where $W_i|, i = 1, 2, 3, are Brownian motions with correlation coefficients $corr(dW_{1,t}, dW_{i,t}$) = 0, i = 2, 3, while q is a Poisson process uncorrelated with $W_i, i = 1, 2, 3$, and governed by the jump
intensity parameter $\lambda$, i.e., $Prob(dq_t = 1) = \lambda$. In the event that $dq_t = 1$, the short rate is subject to a jump, which is expressed as a fraction $(e^{Z_t} − 1)$ of the current rate $r_t$, where $Z_t ~ N(\mu_J,\sigma_J)$ is independent of q and $W_1-W_3$. It then follows that, conditional on a jump or equivalently $dq_t = 1$, the
average jump size is

$κ = E(e^{Z_t} − 1) = e(\mu_J +\sigma^2_J /2) − 1 (4)$

Within the family (1)-(3), we recognize several well-known special cases. If we assume that the
volatility and mean are constant, $\sqrt{V_t = \sigma$ and $\mu_t = \mu}, and exclude jumps, $\lambda = 0$ we obtain the one-factor CKLS model: 
   $dr_t = \kappa_1(\mu − r_t) dt + \sigma r^\gamma_t dW_{1,t }, \gamma ≥ 0 . (5)$
   
According to this representation, $r_t} reverts towards the mean level $\mu$, with $\kappa_1$ measuring the speed of the mean reversion. Subject to appropriate parameter restrictions, the presence of the rγ
t term in the diffusion coefficient rules out negative rates and induces conditional heteroskedasticity, as volatility depends on the level of the short rate, the so-called level effect. Further imposing γ = 0.5 results in the affine one-factor Cox, Ingersoll, and Ross (1985) (CIR) model, while γ = 0 produces the Vasicek (1977) specification.
Relaxing the restriction that $\mu$ is constant yields the “central tendency” model, which is a special
case of (1) given by

   $dr_t = \kappa_1(\mu − r_t dt + \sigma  r^\gamma_t  , \gamma ≥ 0 , (6)$
   
in combination with (3). The specification (6) and (3) allows for a time-varying mean level of the
short rate to dictate the short-run mean reversion in the drift. This extension is consistent with
the indications in Figure 1 that, while there are extended periods of strong drift, mean reversion is
not simply related to a pull towards the overall mean of around 5.6%. The series often appear to
drift downwards, even when rates are considerably below the mean, and to drift upwards even when
rates are well above the mean. It thus appears as if $r_t$ converges rather quickly towards a time-varying mean level, $\mu_t$, whereas $\mu_t$ itself reverts more slowly towards the unconditional mean $\vartheta$.  

An obvious interpretation of the time-varying mean is that of an (expected) inflation factor that, in accordance with the Fisher effect, manifests itself in the required nominal interest rate. Further drift may be induced by a slow evolution in the equilibrium real interest rate

See  Piazzesi (2003b) on affine models
\begin{equation}

dr_t=\kappa_1(\mu_t-\frac{\mu_J \lamba}{kappa_1}-r_t)dt +\sqrt{V,t}dW_{1,t}+Z_tdq_t  (1)
dV_t=\kappa_2(\alpha-\mu_t)dt +\eta_1\sqrt{V_t}dW_{2,t}                              (2)
d\mu_t=\kappa_3(\nu-\mu_J)dt +\eta_3\sqrt{\mu_t}dW_{3,t}                             (3)
\end{equation}

$W_i|, i = 1, 2, 3$, are Brownian motions with correlation coefficients $corr(dW_{1,t}, dW{i,t}) = \rho_{1,i}, i = 2, 3$


## Empirical Results
In this section, we report on our EMM implementation. Section 4.1 outlines the semi-nonparametric
(SNP) model selection procedure used to obtain the moment conditions for the subsequent EMM
estimation step. Section 4.2 details our efficient method of moments (EMM) estimation and inference results.
...
Next,
1.  we use a Gaussian leading term, designed to capture the bulk of the dependency in the conditional mean and variance of the series
2. we allow a squared Hermite polynomial expansion to accommodate any remaining non-normality and
possible time-series dependency in the innovation process. Choose EGARCH motivated no by goodness-of-fit
criteria, and by the analysis in Andersen and Lund (1997), which shows that an EGARCH score
generator has better “dynamic stability” properties than, e.g., a GARCH model, a consideration
which proves important in the EMM implementation. 
3. Finally, we allow for an additional source of
...
this leads to the class of SNP densities:

\begin{align*}
f_K(r_t|x_t; \xi) = 
 \left(\nu + (1 − \nu ) \times \frac{\abs{P_K(z_t, x_t)}^2 }{ \int_ \abs{P_K(z_t, x_t)}^2 \fi(u)dU} \right \frac{\fi(z_r)}{r^{\delta}_{t-1}\sqrt{h_t}}   \nu=.01
\end{align*}

where $\fi(.)$ is the standard normal density, $x_t = {r_1,\cdot,r_{t−1}$ reflects the information set, $\xi$ is the 11 SNP density parameter vector (page 11)

\begin {align*}
z_t = \frac{r_t − \mu_t}{r^δ_{t−1}\sqrt{h_t}}

\mu_t=\phi_0+ \sum_{i=1}^{}\phi(i)r_{t-1}   + \sum_{i=1}^{u} \zeta(r_{t-i}-\mu_{t-i}    )

ln(h_t)=\omega (1-\sum_{i=1}^{p}\beta_i)+\sum_{i=1}^{p}\beta_i ln(h_{t-i}+ (1 + \alpha_1 + \cdot + alpha_q)
) [\theta_1 z_{t−1} + \theta_2 (b(z_{t−1}) −\sqrt{\frac{2}{\pi}}]

 ,
b(z) = \abs{z} for \abs{z} ≥ \frac{\pi}{2K}, b(z)=(\frac{\pi}{2} − \frac{cos(Kz)}{K} for \abs{z} <  \frac{\pi}{2K} 

PK(z, x) = \sum_{i=0}^{K_z}\alpha_i(x)z^i = \sum_{i=0}^{K_z} (\sum_{\abs{j}=0}^{K_x}\alpha{i,j}x^j)z^i
\end{align*}

$z_i , \alpha_{0,0} = 1$ ,
where j is a multi-index vector, $x_j=(x^{j_1}_1, \cdot,x^{j_M}_M)$, and $\abs{j} ≡ \sum_{m=1}^{M}j_m$

As in Andersen and Lund (1997), b(z) is a smooth, twice-differentiable, function that closely approximates the absolute value operator in the EGARCH variance equation, with K = 100

<!--
Notes on parameters:
Stylized Facts
Some phenomena are systematically observed in almost all return time series. A good conditional heteroskedasticity model should be able to capture most of these empirical facts. In this section we list the most well known stylized facts in volatility analysis.

- Volatility Clusters
The volatility is more likely to be high at time t
 if it was also high at time t−1
. That is, a shock at time t−1
 increases not only the variance at time t−1
 but also the variance at time t
. In other words, the markets are more volatile in some periods, and they are more tranquil in others. The volatilities are clustered in time. The GARCH
 type models capture this effect very well. In fact, these models are precisely a way to specify how volatility at time t
 depends on past volatility (and possibly other conditioning variables).

- Fat Tails
Return time series generally present fat tails, also known as excess kurtosis, or leptokurtosis. That is, their kurtosis (the fourth central moment normalized by the square of the variance) is usually greater than three, the kurtosis of a Gaussian random variable. In fact, a popular statistical test for the hypothesis of Gaussianity of a distribution, the Jarque-Bera Test, jointly test both if the distribution is symmetric and if the distribution presents kurtosis equal to three.

If returns are fat-tailed, the probability of having extreme events (very high or very low returns) is higher than it would be in the normal (Gaussian) case.

Most volatility models such as the GARCH model give rise to fat tailed return distributions. This is true whether the underlying shocks are Gaussian or are themselves fat tailed. For estimation it is typically assumed that these underlying shocks are Gaussian, however the estimates are well behaved for large samples even if this is not the case. The estimators are Quasi-Maximum Likelihood or QML estimators in this case and can be shown to be consistent under relatively mild conditions.

- Asymmetry
There is a stylized fact that the plain GARCH
 model is not able to capture, which is the empirically observed fact that negative shocks at time t−1
 have a stronger impact on the variance at time t
 than positive shocks. However, the GARCH
 model can be easily augmented to capture this asymmetry. Examples of these generalizations are the Threshold GARCH
 (TGARCH
), the Asymmetric GARCH
 (AGARCH
) and the Exponential GARCH
 (EGARCH)

- long-run volatility mean, $\omega$
- long run volatility mean, $\alpha$
- volatility-of-volatility coefficient $\eta_1$
- olatility persistence coefficient $\kappa_2$
- e unconditional mean $\zeta$ is 5.7%, virtually indistinguishable
from the 5.6% sample mean.



This asymmetry used to be called leverage the effect because the increase in risk was believed to come from the increased leverage induced by a negative shock, but now it is recognized that this channel does not explain all the facts and risk aversion is an important mechanism.
-Level effect:
The corresponding level coefficient, $\delta$, and the long-run volatility mean, $\omega$,
are highly correlated. The presence of the $r^$\gamma}_t$  term in the
diffusion coefficient rules out negative rates and induces conditional heteroskedasticity, as volatility
depends on the level of the short rate, the so-called level effect. Further imposing γ = 0.5 results in
the affine one-factor Cox, Ingersoll, and Ross (1985) (CIR) model, while γ = 0 produces the Vasicek
(1977) specification.
- Central tendency
Relaxing the restriction that µ is constant yields the “central tendency” model, which is a special
case of (1) The $kappa_1$ coefficient captures the persistence of the short-term interest rate

- The central tendency model is easily extended to the case of a time varying volatility process.
- Jumps. Finally, extending the stochastic volatility and mean drift model to allow for jumps produces
the general non-affine specification (1)-(3). Theory predicts that an unexpected information arrival
should induce a discontinuity in the return process. For example, jumps in interest rates could be
induced by an announced shift in monetary policy. Moreover, from a statistical perspective, adding
a jump component may improve the descriptive power of our models as jumps can accommodate
outliers in the short rate distribution. 
In order to preserve the affine structure of the model, we fix the $\gamma$ coefficient
at zero, thus suppressing the level effect. Further, in the system (8)-(10) the Brownian motion
shocks are uncorrelated and jumps are normally distributed. This implies that the short rate can
become negative. 
-->

<!--
EMM Galant and Tauchen.
estimation and analysis of partially observed nonlinear systems as in the finance continuous time stochastic volatility model, a system of equations describing a security price and its variance. Only the security price is observable.

System parameters are estimated by simulated method of moments EMM, match moments implied by the system to moments implied by the transition density for observables.

Score function is The score function $u(\theta)$ is the partial derivativeof the log-likelihood function $F(\theta)=lnL(\theta)$, where $L(\theta)$ is the standard likelihood function.
\url{https://mathworld.wolfram.com/ScoreFunction.html}
-->

4.1 The SNP Mode

We estimate the SNP densities in (11) by (quasi-)maximum likelihood (QML). In this application,
our proxy for the riskless short rate is the three-month Treasury bill yield.

Our model selection procedure within the family of SNP densities (11) is guided by the Bayesian
(BIC) and Hannan-Quinn (H-Q) information criteria; we assign less importance to the commonly used Akaike criterion (AIC) because of its tendency to overparameterize the models. Further, we
pay close attention to the Ljung-Box tests for the autocorrelation of the (raw and squared) residuals,
which provide guidance in detecting possible sources of misspecification in the conditional mean and
variance leading terms of our densities.
Our analysis, summarized in Table 2, points towards an ARMA(4,1)-Level-EGARCH(2,1)-Kz(6)-
Kx(0). 

4.2 EMM Estimation
We denote the parameter vector of our continuous-time model $\psi$ and let ${r_t(\psi$), x_t(\psi$)}
T (N) t=1 $represent
a sample simulated from the model with $x_t(\psi$) = {r_1(\psi$),\cdot,r_{t−1}(\psi$)}$ reflecting the information set.
The EMM estimator of $\psi$ is then defined by
$\psiˆN = arg min \psi mT (N)(\psi, \xi) WN mT (N)(\psi,\xi)$,
where mT (N)(\psi,\xi) is the expectation of the score function, evaluated by Monte Carlo integration at
the quasi-maximum likelihood estimate of the auxiliary model parameter $\xi$,

\begin{align*}
m_T(N)(\psi,\xi) = \frac{1}{T(N)} \sum_{t=2}^{N} \frac{\frac{\partial f}{\partial ln(f_K(r_t(\psi)|x_t}{\partial \xi} x}ln(f_K(r_t(\psi)|x_t}{\xi}))


and the weighting matrix $W_N$ is a consistent estimate of the inverse asymptotic covariance matrix
of the auxiliary score function. Following Gallant and Tauchen (1996), we estimate the covariance
matrix of the auxiliary score from the outer product of the gradient. In simulating the short rate
sequence ${r_t(\psi), x_t_(\psi)}^{T_N}{t=1}

4.2.1 CKLS and CIR Models
Our representations nest a host of special cases that have been studied extensively in the literature.

The one-factor representation of CKLS or CIR form in equation (5).   (poor)
\begin{align*}    
dr_t = \kappa_1(\mu− r_t) dt + \sigma r^{\gamma}_t dW_{1,t} , \gamma ≥ 0 . (5)
\end{align*} 

According to this representation, rt reverts towards the mean level µ, with κ1 measuring the speed of
the mean reversion. Subject to appropriate parameter restrictions, the presence of the rγ
t term in the diffusion coefficient rules out negative rates and induces conditional heteroskedasticity, as volatility depends on the level of the short rate, the so-called level effect. Further imposing γ = 0.5 results in the affine one-factor Cox, Ingersoll, and Ross (1985) (CIR) model, while γ = 0 produces the Vasicek (1977) specification.

Time varying volatility: by our longer sample that refutes the strictly monotone relation between
the interest rate level and volatility. For example, at the very low interest rate levels in the 1950s
and early 1960s the volatility was comparatively high. Likewise, the volatility in the 1970s was much
higher than in the mid 1980s and early 1990s although the level of the short rate was about the same

Relaxing the restriction that µ is constant yields the “central tendency” model, which is a special
case of (1) given by

\begin{align*} 
d\mu_t = κ\kappa_3(\zeta −  ) dt + η2\sqrt{\mu_t}t dW_{3,t} (3)
dr_t = \kappa_1(\mu_t− r_t) dt + \sigma r^{\gamma}_t dW_{1,t} , \gamma ≥ 0 . (6)
\end{align*} 
...
not simply related to a pull towards the overall mean of around 5.6%. The series often appear to
drift downwards, even when rates are considerably below the mean, and to drift upwards even when
rates are well above the mean. It thus appears as if rt converges rather quickly towards a time-varying mean level, µt, whereas µt itself reverts more slowly towards the unconditional mean.

henceforth,
dt = 1/52 and parameter estimates are given for weekly interest rate data expressed in decimal form
on a yearly basis, as is common in the extant literature. The long-term mean µ of the interest
rate is estimated at 5.1%, roughly in line with the 5.6% mean in our sample (Table 1). The κ1
coefficient captures the persistence of the short-term interest rate. Our κ1 estimate implies a first
order autoregressive coefficient of exp(−κ1/52) = 0.9967 at the weekly level, and a half-life of shocks to
8Estimation conducted using considerably longer simulated samples produced nearly identical results. Furthermore, when the data generating process contains a jump component, the simulation step involves an additional layer of approximation as our procedure for generating jumps renders the EMM criterion function discontinuous in the parameter vector, and this creates problems for the numerical minimization of the EMM objective function, as in Andersen, Benzoni and Lund (2002). To avoid this problem jumps are smoothed using a close continuously differentiable approximation, as described in Appendix A.

rt of 4 years, also in line with the stylized features of short-term interest rate series

model’s inability to accommodate the tail behavior of our interest rate data. Indeed, the score components associated with the a20 and a60 terms in the polynomial expansion of the SNP density are highly significant.


4.2.2 Central Tendency
The central-tendency model, given by (6) and (3), extends CIR by adding a stochastic mean factor.
Initial experimentation revealed that the correlation coefficient $\rho_{1,3}$ is insignificant and poorly identified by the SNP score moments. Thus, from here on we fix $\rho_{1,3} = 0$. For the same reason, we impose the identical constraint on the $\rho_{1,2}$ coefficient whenever volatility is stochastic
...
the model is still overwhelmingly rejected. The reasons are readily identified. The moments associated with the higher order SNP coefficients continue to be highly significant, suggesting that the
conditional innovations are no less troublesome than they were for the one-factor model. Hence, while
the central-tendency factor may, indeed, improve upon the characterization of the drift coefficient it
does not help in terms of accommodating the fat tails in the conditional error distribution.


\<!--
The first family of models takes the form:
\begin{align*} 
drt = \kappa_1(\mu_t − \frac{κ \lambda r_t}{\kappa_1} − r_t) dt + \sqrt{V_t} r^{gamma}_t dW_{1,t} + (e^{Z_t} − 1)r_t dq_t , \gamma ≥ 0                             (1)
d\ln{V_t} = \kappa_2(\alpha − \ln{V_t} ) dt + \eta_1 dW_{3,t} (2)
d\mu_t = κ\kappa_3(\zeta −  ) dt + \eta_2 \sqrt{\mu_t}t dW_{3,t} (3)
drt = κ1(µt − κλrt /κ1 − rt) dt +
\end{align*}
-->

4.2.3 Non-Affine Stochastic Volatility
The stochastic volatility model extends the CIR in a different direction, as the second factor now
induces additional variation in the volatility dynamics. We focus initially on the specification given
by (7) and (2) with, for the time being, a constant short-rate mean, i.e., µt = µ. combined with equations (2)-(3)

\begin{align*} 
d\ln{V_t} = \kappa_2(\alpha − \ln{V_t} ) dt + \eta_1 dW_{3,t} (2)
d\mu_t = κ\kappa_3(\zeta −  ) dt + \eta_2 \sqrt{\mu_t}t dW_{3,t} (3)
dr_t = \kappa_1(\mu_t− r_t) dt + \sqrt{V_t} r^{\gamma}_t dW_{1,t} , \gamma ≥ 0 . (7)
\end{align*}

If log-volatility in equation (2) is serially correlated, then ARCH-type effects will interact with the level effect, creating rather complex volatility dynamics.


4.2.6 Affine Stochastic Volatility and Jumps
Motivated by the recent development of the affine modeling paradigm, we now investigate the affine
version of the stochastic volatility model much in parallel with the account provided above. We
initially explore the system (8)-(9) with a constant short rate mean, µt = µ, and no jumps. 

4.2.7 Affine Stochastic Volatility, Mean Drift, and Jump
 volatility process regains its natural interpretation. Our estimate
for κ2 is 1.6271, indicative of high volatility persistence and very close to the value in the non-affine
model. Further, the η1 estimate drops to roughly one-half of the value obtained in the no-jump cas